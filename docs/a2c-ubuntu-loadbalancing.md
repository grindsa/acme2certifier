...
- configure the Django DB handler (optional MySQL settings template)

```bash
# in acme_srv.cfg under [DBhandler]:
#   handler: django
sudo cp /var/www/acme2certifier/examples/django/settings.py \
...
```bash
 docker run -i -p 80:80 -v $PWD/lego:/.lego/ --rm --name lego --network acme goacme/lego run --tls-skip-verify -s https://ub2204-c2.bar.local -a --email "lego@example.com" -d lego01.bar.local --http
```
