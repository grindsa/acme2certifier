...
"""
    ...
    @patch('acme_srv.helper.load_config')
    def test_108_helper_dns_server_list_load(self, mock_load_config):
        """ successful dns_server_list_load with empty Challenge section """
        mock_load_config.return_value = {'Challenge': {}}
        self.assertEqual(['9.9.9.9', '8.8.8.8'], self.dns_server_list_load())

    @patch('acme_srv.helper.load_config')
    def test_109_helper_dns_server_list_load(self, mock_load_config):
    ...
    def test_114_helper_csr_san_get(self):
    ...
    def test_118_helper_csr_san_byte_get(self):
        """ get sans but two sans """
        csr = ''
    ...
    def test_119_helper_csr_san_get(self, mock_csrload):
        """ get sans but three sans """
        csr = 'csr'
    ...
    def test_125_helper_get_url(self):
    ...
    def test_128_helper_cert_dates_get(self):
    ...
    def test_145_helper_signature_check(self):
    ...
    def test_146_helper_signature_check(self):
    ...
    def test_147_helper_signature_check(self):
    ...
    def test_158_sha256_hash_hex(self):
    ...
    def test_166_cert_pem2der(self):
        """ test cert_der2pem """
...
-----END CERTIFICATE-----"""
    ...
    def test_167_helper_cert_extensions_get(self):
        """ test cert_san_get for a single SAN and recode = False"""
        cert = """-----BEGIN CERTIFICATE-----
MIIEZDCCAkygAwIBAgIIe941mx0FQtAwDQYJKoZIhvcNAQELBQAwKjEXMBUGA1UE
CxMOYWNtZTJjZXJ0aWZpZXIxDzANBgNVBAMTBnN1Yi1jYTAeFw0yMTA0MDkxNTUy
...
-----END CERTIFICATE-----"""
    ...
    def test_168_helper_cert_extensions_get(self):
    ...
    def test_182_print_debug(self, mock_print):
        """ test print_debug """
        self.print_debug(False, 'test')
        self.assertFalse(mock_print.called)

    @patch('builtins.print')
    def test_183_print_debug(self, mock_print):
        """ test print_debug """
        self.print_debug(True, 'test')
        self.assertTrue(mock_print.called)

    def test_184_jwk_thumbprint_get(self):
    ...
    def test_185_jwk_thumbprint_get(self, mock_jwk):
    ...
    @patch('jwcrypto.jwk.JWK')
    def test_186_jwk_thumbprint_get(self, mock_jwk):
        """ test jwk_thumbprint_get with  pubkey """
        pub_key = {'pub_key': 'pub_key'}
    ...
    def test_187_allowed_gai_family(self, mock_sock):
        """ test allowed_gai_family """
        self.assertTrue(self.allowed_gai_family())

    def test_188_validate_csr(self):
    ...
    def test_189_servercert_get(self, mock_sock, mock_context, mock_cert, mock_convert):
        """ test servercert get """
    ...
    @patch('socket.socket')
    @patch('socks.socksocket')
    def test_190_servercert_get(self, mock_sock, mock_ssock, mock_context, mock_cert, mock_convert, mock_ipchk):
    ...
    @patch('socket.socket')
    @patch('socks.socksocket')
    def test_191_servercert_get(self, mock_sock, mock_ssock, mock_context, mock_cert, mock_convert):
    ...
    @patch('ssl.SSLContext.wrap_socket')
    @patch('socks.socksocket')
    def test_192_servercert_get(self, mock_sock, mock_context, mock_cert):
    ...
    def test_193_txt_get(self, mock_resolve, mock_res):
    ...
    @patch('dns.resolver.resolve')
    def test_194_txt_get(self, mock_resolve):
        """ successful dns-query returning one txt record """
        resp_obj = Mock()
        resp_obj.strings = ['foo', 'bar']
        mock_resolve.return_value = [resp_obj]
    ...
    @patch('dns.resolver.resolve')
    def test_195_txt_get(self, mock_resolve):
        """ successful dns-query returning one txt record """
        resp_obj1 = Mock()
    ...
    @patch('dns.resolver.resolve')
    def test_196_txt_get(self, mock_resolve):
        """ successful dns-query returning one txt record """
    ...
    def test_215_handle_exception(self):
    ...
    def test_219_ca_handler_load(self, mock_imp):
        """ test ca_handler_load """
    ...
    @patch('importlib.import_module')
    def test_220_ca_handler_load(self, mock_imp):
        """ test ca_handler_load """
        config_dic = {'CAhandler': {'foo': 'bar'}}
    ...
    def test_221_ca_handler_load(self, mock_util):
        """ test ca_handler_load """
    ...
    @patch('importlib.import_module')
    @patch('importlib.util')
    def test_222_ca_handler_load(self, mock_util, mock_imp):
        """ test ca_handler_load """
        config_dic = {'CAhandler': {'handler_file': 'foo'}}
    ...
    @patch('importlib.import_module')
    @patch('importlib.util')
    def test_223_ca_handler_load(self, mock_util, mock_imp):
        """ test ca_handler_load """
        config_dic = {'CAhandler': {'handler_file': 'foo'}}
        mock_util.module_from_spec.side_effect = Exception('exc_mock_util')
        mock_imp.side_effect = Exception('exc_mock_imp')
    ...
    def test_224_eab_handler_load(self):
    ...
    @patch('importlib.import_module')
    def test_225_eab_handler_load(self, mock_imp):
        """ test eab_handler_load """
    ...
    @patch('importlib.import_module')
    def test_226_eab_handler_load(self, mock_imp):
        """ test eab_handler_load """
        config_dic = {'EABhandler': {'foo': 'bar'}}
        mock_imp.return_value = 'foo'
    ...
    @patch('importlib.util')
    def test_227_eab_handler_load(self, mock_util):
        """ test eab_handler_load """
    ...
    @patch('importlib.import_module')
    @patch('importlib.util')
    def test_228_eab_handler_load(self, mock_util, mock_imp):
        """ test eab_handler_load """
        config_dic = {'EABhandler': {'eab_handler_file': 'foo'}}
        mock_util.module_from_spec.side_effect = Exception('exc_mock_util')
        mock_imp.return_value = 'foo'
    ...
    @patch('importlib.import_module')
    @patch('importlib.util')
    def test_229_eab_handler_load(self, mock_util, mock_imp):
        """ test eab_handler_load """
        config_dic = {'EABhandler': {'eab_handler_file': 'foo'}}
        mock_util.module_from_spec.side_effect = Exception('exc_mock_util')
        mock_imp.side_effect = Exception('exc_mock_imp')
    ...
    @patch('importlib.util')
    def test_233_hooks_load(self, mock_util):
        """ test hooks load with hooks but no hooks_file in  config_dic """
        config_dic = {'Hooks': {'hooks_file': 'bar'}}
    ...
    def test_234_error_dic_get(self):
        """ test error_dic_get """
        result = {
    ...
    def test_245_config_check(self):
        """ test config check """
    ...
    def test_246_helper_cert_cn_get(self):
        """ get cn of csr """
        cert = """MIIDDTCCAfWgAwIBAgIBCjANBgkqhkiG9w0BAQsFADAaMRgwFgYDVQQDEw9mb28u
                ZXhhbXBsZS5jb20wHhcNMTkwMTIwMTY1OTIwWhcNMTkwMjE5MTY1OTIwWjAaMRgw
                FgYDVQQDEw9mb28uZXhhbXBsZS5jb20wggEiMA0GCSqGSIb3DQEBAQUAA4IBDwAw
    ...
    def test_255_pembundle_to_list(self):
        """ bundle to list """
        pembundle_to_list = '-----BEGIN CERTIFICATE-----foo\n-----BEGIN CERTIFICATE-----foo1'
        self.assertEqual(['-----BEGIN CERTIFICATE-----foo\n', '-----BEGIN CERTIFICATE-----foo1\n'], self.pembundle_to_list(self.logger, pembundle_to_list))

    def test_256_certid_check(self):
        """ test certid_check """
...
-----END CERTIFICATE-----'''
    ...
    def test_259_certid_hex_get(self):
        """ test certid_check """
        certid = 'false'
        renewal_info = 'MFswCwYJYIZIAWUDBAIBBCDhge--b3rj6nHHj8meQibXGFcVvj0onqpWgB3_RpbKTQQgrg3PUzRWkYJrgdCT6cdYjDXdXsXqz1sbJgYzBRXV-vQCCCzKhfZA1UFC'
    ...
    def test_267_header_info_get(self):
        """ header_info_get () """
    ...
    def test_268_header_info_get(self):
        """ header_info_get () """
        models_mock = MagicMock()
    ...
    def test_269_encode_url(self):
        # Test with a simple URL
        url = "www.example.com"
        self.assertEqual(url, self.encode_url(self.logger, url))

    def test_270_encode_url(self):
    ...
    def test_258_cert_ski_pyopenssl_get(self):
        """ test cert_san_get for a multiple SAN of type DNS"""
        cert = """MIIDIzCCAgugAwIBAgICBZgwDQYJKoZIhvcNAQELBQAwGjEYMBYGA1UEAxMPZm9v
                LmV4YW1wbGUuY29tMB4XDTE5MDEyMDE3MDkxMVoXDTE5MDIxOTE3MDkxMVowGjEY
                MBYGA1UEAxMPZm9vLmV4YW1wbGUuY29tMIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8A
    ...
    @patch('acme_srv.helper.cert_load')
    def test_259cert_ski_get(self, mock_load, mock_ski):
        """ test cert_ski_get() """
        cert = 'cert'
    ...
    def test_260cert_ski_get(self, mock_load):
        """ test cert_ski_get() """
        cert = 'cert'
