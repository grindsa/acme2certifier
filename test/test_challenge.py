...
class TestACMEHandler(unittest.TestCase):
    ...
    def test_004_challenge_new_set(self, mock_challenge):
        ...
        mock_challenge.return_value = {'foo' : 'bar'}
        self.assertEqual([{'foo': 'bar'}, {'foo': 'bar'}], self.challenge.new_set('authz_name', 'token', False, 'ip'))
    ...
    @patch('acme_srv.nonce.Nonce.generate_and_add')
    @patch('acme_srv.message.Message.check')
    def test_014_challenge_parse(self, mock_mcheck, mock_nnonce):
    ...
    @patch('acme_srv.message.Message.check')
    def test_015_challenge_parse(self, mock_mcheck, mock_cname, mock_nnonce):
    ...
    @patch('acme_srv.challenge.Challenge._name_get')
    @patch('acme_srv.message.Message.check')
    def test_016_challenge_parse(self, mock_mcheck, mock_cname, mock_info, mock_nnonce):
    ...
    @patch('acme_srv.challenge.Challenge._name_get')
    @patch('acme_srv.message.Message.check')
    def test_017_challenge_parse(self, mock_mcheck, mock_cname, mock_info, mock_tnauth, mock_nnonce):
    ...
    @patch('acme_srv.challenge.Challenge._name_get')
    @patch('acme_srv.message.Message.check')
    def test_020_challenge_parse(self, mock_mcheck, mock_cname, mock_cinfo, mock_tnauth, mock_nnonce):
    ...
    @patch('acme_srv.challenge.Challenge._name_get')
    @patch('acme_srv.message.Message.check')
    def test_021_challenge_parse(self, mock_mcheck, mock_cname, mock_cinfo, mock_tnauth, mock_nnonce, mock_validate):
    ...
    def test_028_challenge__validate_http_challenge(self, mock_url, mock_resolve):
        ...
        self.assertEqual((False, False), self.challenge._validate_http_challenge('cert_name', 'dns', 'fqdn', 'token', 'jwk_thumbprint'))
    ...
    def test_029_challenge__validate_http_challenge(self, mock_url, mock_resolve):
        ...
        mock_resolve.return_value = ('foo', False)
        self.assertEqual((True, False), self.challenge._validate_http_challenge('cert_name', 'dns', 'fqdn', 'token', 'jwk_thumbprint'))

    @patch('acme_srv.challenge.proxy_check')
    @patch('acme_srv.challenge.fqdn_resolve')
    @patch('acme_srv.challenge.url_get')
    def test_030_challenge__validate_http_challenge(self, mock_url, mock_resolve, mock_proxy):
        """ test Chalölenge.validate_http_challenge() with a correct challenge """
        mock_url.return_value = 'token.jwk_thumbprint'
    ...
    def test_032_challenge__validate_http_challenge(self, mock_url, mock_resolve):
        ...
        self.assertEqual((False, True), self.challenge._validate_http_challenge('cert_name', 'dns', 'fqdn', 'token', 'jwk_thumbprint'))
    ...
    def test_034_challenge__validate_http_challenge(self, mock_url, mock_resolve):
        ...
        mock_url.return_value = 'token.jwk_thumbprint'
        mock_resolve.return_value = ('foo', False)
        self.assertEqual((True, False), self.challenge._validate_http_challenge('cert_name', 'ip', '127.0.0.1', 'token', 'jwk_thumbprint'))
    ...
    def test_035_challenge__validate_http_challenge(self, mock_url, mock_resolve):
        ...
        mock_url.return_value = 'token.jwk_thumbprint'
        mock_resolve.return_value = ('foo', False)
        self.assertEqual((False, True), self.challenge._validate_http_challenge('cert_name', 'unk', '127.0.0.1', 'token', 'jwk_thumbprint'))
        self.assertFalse(mock_resolve.called)
    ...
    def test_036_challenge__validate_dns_challenge(self, mock_dns, mock_code, mock_hash, mock_resolve):
        ...
        mock_resolve.return_value = ('foo', False)
        self.assertEqual((False, False), self.challenge._validate_dns_challenge('cert_name', 'dns', 'fqdn', 'token', 'jwk_thumbprint'))

    @patch('acme_srv.challenge.fqdn_resolve')
    @patch('acme_srv.challenge.sha256_hash')
    @patch('acme_srv.challenge.b64_url_encode')
    @patch('acme_srv.challenge.txt_get')
    def test_037_challenge__validate_dns_challenge(self, mock_dns, mock_code, mock_hash, mock_resolve):
        """ test Chalölenge.validate_dns_challenge() with correct response """
        mock_dns.return_value = ['foo']
        mock_code.return_value = 'foo'
        mock_hash.return_value = 'hash'
        mock_resolve.return_value = ('foo', False)
        self.assertEqual((True, False), self.challenge._validate_dns_challenge('cert_name', 'dns', 'fqdn', 'token', 'jwk_thumbprint'))

    @patch('acme_srv.challenge.fqdn_resolve')
    @patch('acme_srv.challenge.sha256_hash')
    @patch('acme_srv.challenge.b64_url_encode')
    @patch('acme_srv.challenge.txt_get')
    def test_038_challenge__validate_dns_challenge(self, mock_dns, mock_code, mock_hash, mock_resolve):
    ...
    @patch('acme_srv.challenge.b64_url_encode')
    @patch('acme_srv.challenge.txt_get')
    def test_039_challenge__validate_dns_challenge(self, mock_dns, mock_code, mock_hash, mock_resolve):
    ...
    def test_042_challenge__check(self):
    ...
    def test_061_challenge__wcd_manipulate(self):
        """ get fqdn wc manipulation """
        fqdn = 'foo.bar'
        self.assertEqual('foo.bar', self.challenge._wcd_manipulate(fqdn))

    def test_062_challenge__wcd_manipulate(self):
        """ get fqdn wc manipulation """
        fqdn = '*.foo.bar'
        self.assertEqual('foo.bar', self.challenge._wcd_manipulate(fqdn))
    ...
    def test_069_challenge__check(self):
    ...
    def test_070_challenge__update_authz(self):
    ...
    def test_072_challenge__info(self):
    ...
    def test_073_challenge__new(self):
    ...
    def test_074_challenge__update(self):
    ...
    def test_075_challenge__update_authz(self):
    ...
    def test_076_challenge__update_authz(self):
        """ test Challenge._update_authz - dbstore.authorization_update() raises an exception  """
    ...
    @patch('acme_srv.challenge.fqdn_resolve')
    def test_077_challenge__validate_alpn_challenge(self, mock_resolve):
        """ test validate_alpn_challenge fqdn_resolve returned Invalid """
        mock_resolve.return_value = (None, True)
        self.assertEqual((False, True), self.challenge._validate_alpn_challenge('cert_name', 'dns', 'fqdn', 'token', 'jwk_thumbprint'))

    @patch('acme_srv.challenge.servercert_get')
    @patch('acme_srv.challenge.fqdn_resolve')
    def test_078_challenge__validate_alpn_challenge(self, mock_resolve, mock_srv):
        """ test validate_alpn_challenge no certificate returned """
        mock_resolve.return_value = ('foo', False)
        mock_srv.return_value = None
        self.assertEqual((False, False), self.challenge._validate_alpn_challenge('cert_name', 'dns', 'fqdn', 'token', 'jwk_thumbprint'))

    @patch('acme_srv.challenge.proxy_check')
    @patch('acme_srv.challenge.servercert_get')
    @patch('acme_srv.challenge.fqdn_resolve')
    def test_079_challenge__validate_alpn_challenge(self, mock_resolve, mock_srv, mock_proxy):
        """ test validate_alpn_challenge no certificate returned """
        mock_resolve.return_value = ('foo', False)
        mock_srv.return_value = None
    ...
    @patch('acme_srv.challenge.servercert_get')
    @patch('acme_srv.challenge.fqdn_resolve')
    def test_080_challenge__validate_alpn_challenge(self, mock_resolve, mock_srv, mock_sanget, mock_sanchk):
    ...
    @patch('acme_srv.challenge.servercert_get')
    @patch('acme_srv.challenge.fqdn_resolve')
    def test_081_challenge__validate_alpn_challenge(self, mock_resolve, mock_srv, mock_sanget, mock_sanchk, mock_encode, mock_ext):
    ...
    @patch('acme_srv.challenge.servercert_get')
    @patch('acme_srv.challenge.fqdn_resolve')
    def test_082_challenge__validate_alpn_challenge(self, mock_resolve, mock_srv, mock_sanget, mock_sanchk, mock_encode, mock_ext):
        ...
        self.assertEqual((True, False), self.challenge._validate_alpn_challenge('cert_name', 'dns', 'fqdn', 'token', 'jwk_thumbprint'))
    ...
    @patch('acme_srv.challenge.servercert_get')
    @patch('acme_srv.challenge.fqdn_resolve')
    def test_083_challenge__validate_alpn_challenge(self, mock_resolve, mock_srv, mock_sanget, mock_sanchk, mock_encode, mock_ext):
        """ test validate_alpn_challenge extension sucessful """
        mock_resolve.return_value = ('foo', False)
        mock_sanget.return_value = ['foo', 'bar']
        mock_sanchk.return_value = True
    ...
    @patch('acme_srv.challenge.servercert_get')
    @patch('acme_srv.challenge.fqdn_resolve')
    def test_084_challenge__validate_alpn_challenge(self, mock_resolve, mock_srv, mock_sanget, mock_sanchk, mock_encode, mock_ext):
        """ test validate_alpn_challenge extension sucessful """
        mock_encode.return_value = 'foo'
        self.assertEqual((False, True), self.challenge._validate_alpn_challenge('cert_name', 'unk', 'fqdn', 'token', 'jwk_thumbprint'))
        self.assertFalse(mock_resolve.called)
    ...
    @patch('acme_srv.challenge.Challenge._validate')
    def test_085__existing_challenge_validate(self, mock_validate):
    ...
    @patch('acme_srv.challenge.Challenge._validate')
    def test_086__existing_challenge_validate(self, mock_validate):
    ...
    @patch('json.loads')
    @patch('acme_srv.challenge.load_config')
    def test_094_config_load(self, mock_load_cfg, mock_json):
        """ test _config_load two DNS """
        parser = configparser.ConfigParser()
        parser['Challenge'] = {'dns_server_list': '["10.10.10.10", "10.0.0.1"]'}
        mock_load_cfg.return_value = parser
    ...
    @patch('acme_srv.challenge.parse_url')
    def test_103__name_get(self, mock_parse):
    ...
    @patch('acme_srv.challenge.parse_url')
    def test_104__name_get(self, mock_parse):
        """ test name get challenge_path replace """
    ...
    def test_105__validate(self, mock_update, mock_aupdate):
        """ test validate """
    ...
    @patch('acme_srv.challenge.Challenge._update_authz')
    @patch('acme_srv.challenge.Challenge._update')
    def test_106__validate(self, mock_update, mock_aupdate, mock_check):
    ...
    @patch('acme_srv.challenge.Challenge._update_authz')
    @patch('acme_srv.challenge.Challenge._update')
    def test_107__validate(self, mock_update, mock_aupdate, mock_check):
    ...
    @patch('acme_srv.challenge.Challenge._update_authz')
    @patch('acme_srv.challenge.Challenge._update')
    def test_108__validate(self, mock_update, mock_aupdate, mock_check):
    ...
    @patch('acme_srv.challenge.Challenge._update_authz')
    @patch('acme_srv.challenge.Challenge._update')
    def test_109__validate(self, mock_update, mock_aupdate, mock_check):
    ...
    @patch('acme_srv.challenge.Challenge._update_authz')
    @patch('acme_srv.challenge.Challenge._update')
    def test_110__validate(self, mock_update, mock_aupdate, mock_check):
        """ test validate check returned ch:True/inv:False """
        challenge_name = 'challenge_name'
    ...
    @patch('acme_srv.challenge.Challenge._name_get')
    @patch('acme_srv.challenge.Challenge._info')
    def test_111_get(self, mock_info, mock_name):
        """ test get """
    ...
    def test_112_challengeset_get(self, mock_chsearch, mock_val, mock_set):
    ...
    @patch('acme_srv.challenge.Challenge._existing_challenge_validate')
    @patch('acme_srv.challenge.Challenge._challengelist_search')
    def test_113_challengeset_get(self, mock_chsearch, mock_val, mock_set):
