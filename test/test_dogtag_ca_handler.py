...
class TestCAhandler(unittest.TestCase):
    def setUp(self):
        ...
        from examples.ca_handler.dogtag_ca_handler import CAhandler, update_validity_attributes, approve_profile_get
    ...
    def test_002_config_load_missing_keys(self):
        ...
        parser["foo"] = {"foo": "bar"}
        with patch("examples.ca_handler.dogtag_ca_handler.load_config", return_value=parser):
    ...
    @patch("examples.ca_handler.dogtag_ca_handler.config_eab_profile_load", return_value=(True, "handler"))
    ...
    @patch("examples.ca_handler.dogtag_ca_handler.config_profile_load", return_value={"p": 1})
    ...
    @patch("examples.ca_handler.dogtag_ca_handler.config_headerinfo_load", return_value=True)
    ...
    @patch("examples.ca_handler.dogtag_ca_handler.config_enroll_config_log_load", return_value=("log", ["skip"]))
    ...
    def test_017_api_post_creates_session(self):
        ...
        with patch.object(self.cahandler, "_login", side_effect=fake_login) as mock_login:
            code, content = self.cahandler._api_post("/test", {"foo": "bar"})
    ...
    def test_022_api_version_get_success(self):
        ...
        with patch.object(self.cahandler, "_api_get", return_value=(200, version_dict)) as mock_get:
    ...
    def test_024_api_version_get_failure(self):
        self.cahandler.api_version = None
        self.cahandler.REST_INFO = "/info"
        with patch.object(self.cahandler, "_api_get", return_value=(404, {"error": "not found"})) as mock_get:
            with patch.object(self.cahandler.logger, "error") as mock_log:
                self.cahandler._api_version_get()
                mock_get.assert_called_with("/info")
    ...
    def test_026_get_approval_nonce_success(self):
        ...
        with patch.object(self.cahandler, "_api_get", return_value=(200, {"nonce": "abc123"})) as mock_get:
            with patch.object(self.cahandler.logger, "debug") as mock_log:
                ...
                self.assertTrue(any("Received nonce for approval" in msg for msg in calls))

    def test_027_get_approval_nonce_status_logging(self):
        self.cahandler.REST_AGENT_CERTREQUESTS = "/agent/"
        response = {"requestStatus": "pending", "nonce": "abc123"}
        with patch.object(self.cahandler, "_api_get", return_value=(200, response)) as mock_get:
            with patch.object(self.cahandler.logger, "debug") as mock_log:
                error, nonce = self.cahandler._get_approval_nonce("reqid")
                mock_get.assert_called_with("/agent/reqid")
    ...
    def test_028_get_approval_nonce_failure(self):
        ...
        with patch.object(self.cahandler, "_api_get", return_value=(404, {"error": "not found"})) as mock_get:
            with patch.object(self.cahandler.logger, "error") as mock_log:
                error, nonce = self.cahandler._get_approval_nonce("reqid")
                mock_get.assert_called_with("/agent/reqid")
                self.assertEqual(error, "Failed to get nonce for certificate request approval.")
    ...
    def test_029_get_approval_nonce_no_nonce(self):
        ...
        with patch.object(self.cahandler, "_api_get", return_value=(200, {"foo": "bar"})):
            with patch.object(self.cahandler.logger, "error") as mock_log:
                error, nonce = self.cahandler._get_approval_nonce("reqid")
                self.assertEqual(error, "Failed to get nonce for certificate request approval.")
                self.assertIsNone(nonce)
    ...
    def test_030_certrequest_approve_nonce_error(self):
        self.cahandler.REST_AGENT_CERTREQUESTS = "/agent/"
        with patch.object(self.cahandler, "_get_approval_nonce", return_value=("nonce error", None)) as mock_nonce:
            with patch.object(self.cahandler.logger, "error") as mock_log:
                error, cert_bundle, cert_raw = self.cahandler._certrequest_approve("reqid")
                ...
                calls = [c[0][0] for c in mock_log.call_args_list]
                self.assertTrue(any("Cannot approve certificate request" in msg for msg in calls))

    def test_031_certrequest_approve_missing_certId(self):
        self.cahandler.REST_AGENT_CERTREQUESTS = "/agent/"
        self.cahandler.REST_CERTREQUESTS = "/certreqs"
        with patch.object(self.cahandler, "_get_approval_nonce", return_value=(None, "nonce")):
            with patch("examples.ca_handler.dogtag_ca_handler.approve_profile_get", return_value={}):
                with patch.object(self.cahandler, "_api_post", return_value=(200, {})):
                    with patch.object(self.cahandler, "_api_get", return_value=(200, {"requestStatus": "approved"})):
                        error, cert_bundle, cert_raw = self.cahandler._certrequest_approve("reqid")
    ...
    def test_032_certrequest_approve_no_request_status(self):
        ...
        with patch.object(self.cahandler, "_get_approval_nonce", return_value=(None, "nonce")):
            with patch("examples.ca_handler.dogtag_ca_handler.approve_profile_get", return_value={}):
                with patch.object(self.cahandler, "_api_post", return_value=(200, {})):
                    with patch.object(self.cahandler, "_api_get", return_value=(200, {})):
                        with patch.object(self.cahandler.logger, "error") as mock_log:
                            error, cert_bundle, cert_raw = self.cahandler._certrequest_approve("reqid")
                            self.assertIn("Failed to approve certificate request", error)
                            self.assertIsNone(cert_bundle)
                            self.assertIsNone(cert_raw)
                            # Check log message for no requestStatus (line 417)
                            calls = [c[0][0] for c in mock_log.call_args_list]
                            self.assertTrue(any("Failed to get certificate request status after approval attempt" in msg for msg in calls))
        ...
        with patch.object(self.cahandler, "_get_approval_nonce", return_value=(None, "nonce")):
            with patch("examples.ca_handler.dogtag_ca_handler.approve_profile_get", return_value={}):
                with patch.object(self.cahandler, "_api_post", return_value=(200, {})):
                    ...
                    with patch.object(self.cahandler, "_api_get", side_effect=[
                        (200, {"requestStatus": "approved", "certId": "cid"}),
                        (200, {"PKCS7CertChain": "chaindata"})
                    ]):
                        with patch("examples.ca_handler.dogtag_ca_handler.pkcs7_to_pem", return_value="pemchain"):
                            with patch("examples.ca_handler.dogtag_ca_handler.b64_decode", return_value=b"bytes"):
                                with patch("examples.ca_handler.dogtag_ca_handler.cert_pem2der", return_value=b"der"):
                                    with patch("examples.ca_handler.dogtag_ca_handler.b64_encode", return_value="rawcert"):
                                        with patch.object(self.cahandler.logger, "error") as mock_log:
                                            error, cert_bundle, cert_raw = self.cahandler._certrequest_approve("reqid")
                                            ...
                                            calls = [c[0][0] for c in mock_log.call_args_list]
                                            self.assertTrue(any("failed to fetch raw certificate data" in msg for msg in calls))
            ...
            with patch("examples.ca_handler.dogtag_ca_handler.approve_profile_get", return_value={}):
                with patch.object(self.cahandler, "_api_post", return_value=(200, {})):
                    ...
                    with patch.object(self.cahandler, "_api_get", side_effect=[
                        ...
                        with patch("examples.ca_handler.dogtag_ca_handler.pkcs7_to_pem", return_value=None):
                            with patch("examples.ca_handler.dogtag_ca_handler.b64_decode", return_value=b"bytes"):
                                with patch("examples.ca_handler.dogtag_ca_handler.cert_pem2der", return_value=b"der"):
                                    with patch("examples.ca_handler.dogtag_ca_handler.b64_encode", return_value="rawcert"):
                                        with patch.object(self.cahandler.logger, "error") as mock_log:
                                            error, cert_bundle, cert_raw = self.cahandler._certrequest_approve("reqid")
                                            self.assertIn("certificate chain data", error)
                                            self.assertIsNone(cert_bundle)
                                            self.assertEqual(cert_raw, "rawcert")
                                            # Check log message for chain error (line 415)
                                            calls = [c[0][0] for c in mock_log.call_args_list]
                                            self.assertTrue(any("failed to fetch certificate chain data" in msg for msg in calls))
    ...
    def test_036_parse_cert_key_usage_and_eku(self):
        ...
        with patch("cryptography.x509.load_pem_x509_certificate", return_value=DummyCert()):
    ...
    def test_039_certrequest_send_success(self):
        ...
        mock_response = {
            "entries": [
                {"requestId": "req123", "requestStatus": "pending"}
            ]
        }
        with patch.object(self.cahandler, "_api_post", return_value=(200, mock_response)):
            with patch.object(self.cahandler.logger, "info") as mock_log:
                ...
                mock_log.assert_any_call("Request ID: %s, Request Status: %s", "req123", "pending")
    ...
    def test_041_certrequest_send_failed_logs_error(self):
        ...
        with patch.object(self.cahandler, "_api_post", return_value=(500, mock_response)):
            with patch.object(self.cahandler.logger, "error") as mock_log:
                req_id, req_status = self.cahandler._certrequest_send("csrdata")
                self.assertIsNone(req_id)
                self.assertIsNone(req_status)
    ...
    def test_042_config_passphrase_load_env_error_logs(self):
        ...
        config_dic = {
            "CAhandler": {
                "cert_passphrase_variable": "MISSING_ENV_VAR"
            }
        }
    ...
    def test_045_login_pkcs12_auth(self):
        ...
        with patch("requests.Session") as mock_session, \
             patch("examples.ca_handler.dogtag_ca_handler.Pkcs12Adapter") as mock_adapter, \
            ...
            mock_adapter.assert_called_once_with(pkcs12_filename="dummy_cert.p12", pkcs12_password="dummy_pass")
            mock_sess_instance.mount.assert_called_once_with("https://dummyhost", mock_adapter.return_value)
            mock_sess_instance.headers.update.assert_called_once_with({"Accept": "application/json"})
            mock_log.assert_any_call("CAhandler._login()")
            mock_log.assert_any_call("CAhandler._login() using PKCS12 client authentication")
    ...
    def test_046_login_pem_auth(self):
        ...
        with patch("requests.Session") as mock_session, \
            ...
            self.cahandler._login()
            mock_session.assert_called_once()
            self.assertEqual(mock_sess_instance.cert, ("dummy_cert.pem", "dummy_key.pem"))
            mock_sess_instance.headers.update.assert_called_once_with({"Accept": "application/json"})
            mock_log.assert_any_call("CAhandler._login()")
    ...
    def test_048_revoke_success_logs_info(self):
        ...
        with patch.object(self.cahandler, "_api_get", return_value=(200, response)), \
             patch.object(self.cahandler, "_api_post", return_value=(200, {})), \
            ...
            mock_info.assert_any_call("Certificate with serial %s revoked successfully", serial)
    ...
    def test_049_revoke_failed_revoke_logs_error(self):
        ...
        response = {"Nonce": "nonce", "Status": "valid"}
        with patch.object(self.cahandler, "_api_get", return_value=(200, response)), \
             patch.object(self.cahandler, "_api_post", return_value=(500, {"error": "fail"})), \
    ...
    def test_050_revoke_already_revoked_logs_info(self):
        ...
        with patch.object(self.cahandler, "_api_get", return_value=(200, response)), \
            ...
            mock_info.assert_any_call("Certificate with serial %s is already revoked", serial)

    def test_051_revoke_status_fail_logs_error(self):
        """Test _revoke logs error if status fetch fails (line 687)"""
        serial = "0x1234"
        with patch.object(self.cahandler, "_api_get", return_value=(500, {"error": "fail"})), \
             patch.object(self.cahandler.logger, "error") as mock_error:
            code, msg, detail = self.cahandler._revoke(serial)
            self.assertEqual(code, 500)
            self.assertEqual(msg, "Failed to get certificate status before revocation attempt")
            mock_error.assert_any_call(
    ...
    def test_052_revoke_prepends_0x_to_serial(self):
        ...
        response = {"Nonce": "nonce", "Status": "valid"}
        with patch.object(self.cahandler, "_api_get", return_value=(200, response)) as mock_get, \
             patch.object(self.cahandler, "_api_post", return_value=(200, {})) as mock_post, \
             patch.object(self.cahandler.logger, "info") as mock_info:
            code, msg, detail = self.cahandler._revoke(serial)
            self.assertEqual(code, 200)
            self.assertEqual(msg, "Certificate revoked successfully")
            # Ensure _api_get was called with the serial prefixed by '0x'
            mock_get.assert_called_once_with(f"{self.cahandler.REST_AGENT_CERTS}{expected_serial}")
            mock_info.assert_any_call("Certificate with serial %s revoked successfully", expected_serial)
    ...
    def test_053_enroll_error_from_eab_profile_header_info_check(self):
        ...
        with patch("examples.ca_handler.dogtag_ca_handler.eab_profile_header_info_check", return_value="some error") as mock_eab, \
             patch.object(self.cahandler, "_api_version_get") as mock_ver, \
             patch.object(self.cahandler, "_certrequest_send") as mock_send:
    ...
    def test_054_enroll_success_pending_certrequest_approve(self):
        ...
        with patch("examples.ca_handler.dogtag_ca_handler.eab_profile_header_info_check", return_value=None), \
             patch.object(self.cahandler, "_api_version_get"), \
             patch.object(self.cahandler, "_certrequest_send", return_value=("reqid", "pending")), \
             patch.object(self.cahandler, "_certrequest_approve", return_value=("err", "bundle", "raw")) as mock_approve:
            error, cert_bundle, cert_raw, poll_identifier = self.cahandler.enroll("csr")
    ...
    def test_056_enroll_success_not_pending(self):
        ...
        with patch("examples.ca_handler.dogtag_ca_handler.eab_profile_header_info_check", return_value=None), \
             patch.object(self.cahandler, "_api_version_get"), \
             patch.object(self.cahandler, "_certrequest_send", return_value=("reqid", "issued")):
            error, cert_bundle, cert_raw, poll_identifier = self.cahandler.enroll("csr")
            self.assertIsNone(error)
            self.assertIsNone(cert_bundle)
            self.assertIsNone(cert_raw)
    ...
    def test_057_enroll_enrollment_config_log_called(self):
        ...
        with patch("examples.ca_handler.dogtag_ca_handler.eab_profile_header_info_check", return_value=None), \
             patch.object(self.cahandler, "_api_version_get"), \
             patch.object(self.cahandler, "_certrequest_send", return_value=("reqid", "pending")), \
             patch("examples.ca_handler.dogtag_ca_handler.enrollment_config_log") as mock_log:
            self.cahandler.enrollment_config_log_skip_list = ["foo"]
            self.cahandler.enroll("csr")
            mock_log.assert_called_once_with(self.cahandler.logger, self.cahandler, ["foo"])

    def test_058_handler_check_success(self):
        """Test handler_check returns None for valid config and logs debug"""
        with patch("examples.ca_handler.dogtag_ca_handler.handler_config_check", return_value=None) as mock_check, \
             patch.object(self.cahandler.logger, "debug") as mock_log:
            result = self.cahandler.handler_check()
            self.assertIsNone(result)
            mock_check.assert_called_once_with(self.cahandler.logger, self.cahandler, ["api_host", "client_cert"])
    ...
    def test_059_handler_check_error(self):
        ...
        with patch("examples.ca_handler.dogtag_ca_handler.handler_config_check", return_value="error") as mock_check, \
             patch.object(self.cahandler.logger, "debug") as mock_log:
            result = self.cahandler.handler_check()
    ...
    def test_062_revoke_success(self):
        ...
        with patch("examples.ca_handler.dogtag_ca_handler.cert_serial_get", return_value="serial") as mock_serial, \
             patch.object(self.cahandler, "_revoke", return_value=(200, "ok", "")) as mock_revoke, \
             patch.object(self.cahandler.logger, "debug") as mock_debug:
            code, msg, detail = self.cahandler.revoke("certdata")
            self.assertEqual((code, msg, detail), (200, "ok", ""))
            mock_serial.assert_called_once_with(self.cahandler.logger, "certdata", hexformat=True)
    ...
    def test_063_revoke_serial_extraction_fails(self):
        ...
        with patch("examples.ca_handler.dogtag_ca_handler.cert_serial_get", return_value=None), \
             patch.object(self.cahandler.logger, "error") as mock_error, \
             patch.object(self.cahandler.logger, "debug") as mock_debug:
            code, msg, detail = self.cahandler.revoke("certdata")
            self.assertEqual(code, 400)
            self.assertEqual(msg, "urn:ietf:params:acme:error:malformed")
            self.assertEqual(detail, "Invalid certificate format or missing serial number")
            mock_error.assert_any_call("Failed to extract serial number from certificate for revocation")
            mock_debug.assert_any_call("CAhandler.revoke()")
            mock_debug.assert_any_call("Certificate.revoke() ended")
    ...
    def test_065_revoke_logs_debug(self):
        ...
        with patch("examples.ca_handler.dogtag_ca_handler.cert_serial_get", return_value="serial"), \
    ...
    def test_066_enter_calls_config_load_and_login_if_no_api_host(self):
        ...
        self.cahandler.api_host = None
        with patch.object(self.cahandler, "_config_load") as mock_config, \
             patch.object(self.cahandler, "_login") as mock_login:
    ...
    def test_074_approve_profile_get_default(self):
        ...
        self.assertIn("notBefore", [a["name"] for a in result["ProfilePolicySet"][0]["policies"][0]["def"]["attributes"]])
        self.assertIn("notAfter", [a["name"] for a in result["ProfilePolicySet"][0]["policies"][0]["def"]["attributes"]])
        calls = [c[0][0] for c in mock_logger.debug.call_args_list]
    ...
    def test_075_approve_profile_get_with_nonce_and_request_id(self):
        ...
        mock_logger = Mock()
        result = self.approve_profile_get(mock_logger, nonce="abc123", request_id="req42")
    ...
    def test_076_approve_profile_get_notbefore_notafter_values(self):
        ...
        with patch("examples.ca_handler.dogtag_ca_handler.uts_now", return_value=1000000), \
             patch("examples.ca_handler.dogtag_ca_handler.uts_to_date_utc", side_effect=["NB", "NA"]):
    ...
    def test_077_approve_profile_get_does_not_log_nonce(self):
        ...
        mock_logger = Mock()
        self.approve_profile_get(mock_logger, nonce="shouldnotappear", request_id="reqid")
