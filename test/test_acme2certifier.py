...
class TestACMEHandler(unittest.TestCase):
    ...
    @patch('acme.message.Message.check')
    def test_152_new_post(self, mock_mcheck, mock_certget, mock_response):
    ...
    @patch('acme.nonce.Nonce.generate_and_add')
    def test_153_prepare_response(self, mock_nnonce):
    ...
    @patch('acme.nonce.Nonce.generate_and_add')
    def test_154_prepare_response(self, mock_nnonce, mock_error):
    ...
    @patch('acme.error.Error.enrich_error')
    @patch('acme.nonce.Nonce.generate_and_add')
    def test_158_prepare_response(self, mock_nnonce, mock_error):
    ...
    @patch('acme.message.Message.check')
    def test_174_order_parse(self, mock_mcheck, mock_oname):
    ...
    @patch('acme.order.Order._name_get')
    @patch('acme.message.Message.check')
    def test_175_order_parse(self, mock_mcheck, mock_oname, mock_lookup):
    ...
    @patch('acme.order.Order._name_get')
    @patch('acme.message.Message.check')
    def test_176_order_parse(self, mock_mcheck, mock_oname, mock_lookup, mock_process):
    ...
    @patch('acme.message.Message.check')
    def test_181_authorization_post(self, mock_mcheck):
    ...
    @patch('acme.message.Message.check')
    def test_182_authorization_post(self, mock_mcheck, mock_authzinfo):
    ...
    @patch('acme.authorization.Authorization._authz_info')
    @patch('acme.message.Message.check')
    def test_183_authorization_post(self, mock_mcheck, mock_authzinfo, mock_nnonce):
        """ Authorization.new_post() failed bcs url is missing in protected """
    ...
    def test_184_cert_serial_get(self):
    ...
    def test_187_cert_serial_get(self):
    ...
    def test_201_revocation_request_validate(self):
    ...
    @patch('acme.message.Message.check')
    def test_208_revoke(self, mock_mcheck, mock_validate):
...
"""
    ...
    @patch('acme.authorization.uts_now')
    @patch('acme.authorization.generate_random_string')
    def test_259_authorization_info(self, mock_name, mock_uts, mock_challengeset):
    ...
    @patch('acme.authorization.Authorization._authz_info')
    @patch('acme.message.Message.check')
    def test_260_authorization_post(self, mock_mcheck, mock_authzinfo, mock_nnonce):
        """ Authorization.new_post() failed bcs url is missing in protected """
        mock_mcheck.return_value = (200, None, None, {'url' : 'foo_url'}, 'payload', 'account_name')
    ...
    @patch('acme.account.Account._contact_check')
    def test_263_account_contact_update(self, mock_contact_chk,):
    ...
    @patch('acme.message.Message.check')
    def test_264_accout_parse(self, mock_mcheck, mock_contact_upd):
    ...
    @patch('acme.account.Account._contacts_update')
    @patch('acme.message.Message.check')
    def test_265_accout_parse(self, mock_mcheck, mock_contact_upd, mock_account_lookup, mock_datestr, mock_nnonce):
    ...
    def test_268_date_to_datestr(self):
    ...
    def test_271_datestr_to_date(self):
    ...
    def test_296_inner_payload_check(self):
    ...
    def test_297_inner_payload_check(self, mock_cmp):
        """ Account.inner_payload_check() with same kid and account values but no old_key"""
        outer_protected = {'kid': 'kid'}
    ...
    def test_298_key_change_validate(self):
    ...
    @patch('acme.account.Account._lookup')
    def test_299_key_change_validate(self, mock_lup):
    ...
    @patch('acme.account.Account._lookup')
    def test_300_key_change_validate(self, mock_lup, mock_jws_chk):
    ...
    @patch('acme.account.Account._inner_jws_check')
    @patch('acme.account.Account._lookup')
    def test_301_key_change_validate(self, mock_lup, mock_jws_chk, mock_pl_chk):
    ...
    @patch('acme.message.Message.check')
    def test_304_key_change(self, mock_mcheck):
    ...
    @patch('acme.account.Account._key_change_validate')
    @patch('acme.message.Message.check')
    def test_307_key_change(self, mock_mcheck, moch_kchval):
        """ Account.key_change() - account_update returns nothing"""
        protected = {'url': 'url/key-change'}
        mock_mcheck.return_value = (200, 'message1', 'detail1', {'jwk': {'h1': 'h1a', 'h2': 'h2a', 'h3': 'h3a'}}, 'payload', 'aname')
        moch_kchval.return_value = (200, 'message2', 'detail2')
    ...
    def test_310_order_process(self):
    ...
    @patch('acme.order.Order._info')
    def test_314_order_process(self, mock_info):
    ...
    @patch('acme.order.Order._info')
    def test_315_order_process(self, mock_info, mock_process_csr):
    ...
    @patch('acme.order.Order._csr_process')
    @patch('acme.order.Order._info')
    def test_316_order_process(self, mock_info, mock_process_csr, mock_update):
        """ Order.prcoess() finalize request with CSR but csr_process failed """
        mock_info.return_value = {'status': 'ready'}
        order_name = 'order_name'
        protected = {'url': {'finalize': 'foo', 'foo': 'bar'}}
    ...
    def test_317_cert_pubkey_get(self):
...
"""
    ...
    def test_318_csr_pubkey_get(self):
        """ test get public_key from certificate """
        csr = """"""
...
"""
    ...
    def test_327_convert_byte_to_string(self):
        """ convert byte2string for a string value """
    ...
    @patch('acme.trigger.Trigger._payload_process')
    def test_332_parse(self, mock_process):
    ...
    def test_347_wcd_manipulate(self):
        """ get fqdn wc manipulation """
        fqdn = 'foo*.foo.bar'
    ...
    def test_352_convert_string_to_byte(self):
        """ convert string value to byte """
        value = None
    ...
    @patch('acme.helper.load_config')
    def test_371_dns_server_list_load(self, mock_load_config):
        """ successful dns_server_list_load with wrong json format """
    ...
    @patch('acme.account.generate_random_string')
    def test_374_account_add_new(self, mock_name):
        """ test account add without contact """
    ...
    @patch('acme.message.Message.check')
    def test_375_account_new(self, mock_mcheck):
    ...
    @patch('acme.account.Account._contact_check')
    @patch('acme.message.Message.check')
    def test_376_account_new(self, mock_mcheck, mock_contact, mock_aad, mock_nnonce):
    ...
    @patch('acme.account.Account._contact_check')
    @patch('acme.message.Message.check')
    def test_377_account_new(self, mock_mcheck, mock_contact, mock_aad, mock_nnonce):
    ...
    @patch('acme.account.Account._add')
    @patch('acme.message.Message.check')
    def test_378_account_new(self, mock_mcheck, mock_aad, mock_nnonce):
    ...
    @patch('acme.account.Account._add')
    @patch('acme.message.Message.check')
    def test_379_account_new(self, mock_mcheck, mock_aad, mock_nnonce, mock_tos):
    ...
    def test_381_get_dir_meta(self):
        """ test Directory.get_directory() method and check for "meta" tag in output"""
    ...
    def test_382_get_dir_meta(self):
        """ test Directory.get_directory() method and check for "meta" tag in output"""
        self.directory.version = '0.1'
        self.directory.tos_url = 'foo'
    ...
    @patch('acme.message.Message.check')
    def test_383_account_new(self, mock_mcheck):
    ...
    def test_459_cert_dates_get(self):
