...
class TestACMEHandler(unittest.TestCase):
    ...
    def test_012_renewalinfo_get(self, mock_lookup, mock_gen):
        ...
        self.assertEqual(
            {"foo": "bar"}, self.renewalinfo._renewalinfo_get("1a2b3c4d5e6f")
        )
    ...
    def test_013_get(self, mock_update, mock_renstr_get, mock_renget):
        ...
        self.assertEqual(
            {"code": 200, "data": {"foo": "bar"}, "header": {"Retry-After": "86400"}},
            self.renewalinfo.get("url"),
        )
    ...
    def test_015_get(self, mock_update, mock_renstr_get, mock_renget):
        ...
        self.renewalinfo.dbstore.hkparameter_get.return_value = None
        self.assertEqual(
            {"code": 404, "data": "urn:ietf:params:acme:error:malformed"},
            self.renewalinfo.get("url"),
        )
        self.assertTrue(mock_update.called)
    ...
    def test_016_get(self, mock_update, mock_renstr_get, mock_renget):
        ...
        with self.assertLogs("test_a2c", level="INFO") as lcm:
            self.assertEqual(
                {"code": 400, "data": "urn:ietf:params:acme:error:malformed"},
                self.renewalinfo.get("url"),
            )
        self.assertTrue(mock_update.called)
        self.assertTrue(mock_renstr_get.called)
    ...
    def test_026_cert_dic_lookup(
        self, mock_aki, mock_certid, mock_draft01, mock_draft02
    ):
        ...
        self.assertEqual(
            {"foo": "draft02"}, self.renewalinfo._cert_dic_lookup("cert.id")
        )
    ...
    def test_027_cert_dic_lookup(
        ...
        mock_draft02.return_value = {"foo": "draft02"}
        self.assertEqual(
            {"foo": "draft01"}, self.renewalinfo._cert_dic_lookup("certid")
        )
    ...
    def test_029_cert_table_update(self, mock_serial, mock_aki):
        ...
        self.renewalinfo.dbstore.certificates_search.return_value = [
            {"foo": "bar", "cert_raw": "cert_raw", "name": "name", "cert": "cert"}
        ]
    ...
    def test_030_cert_table_update(self, mock_serial, mock_aki):
        """test _cert_table_update()"""
        self.renewalinfo.dbstore.certificates_search.return_value = [
            {"foo": "bar", "cert_raw": None, "name": "name", "cert": "cert"}
        ]
        self.assertFalse(self.renewalinfo._cert_table_update())
    ...
    def test_033_draft01_lookup(self):
        """test _draft01_lookup()"""
        self.renewalinfo.dbstore.certificate_lookup.side_effect = Exception(
            "cert_lookup"
        )
        with self.assertLogs("test_a2c", level="INFO") as lcm:
    ...
    def test_034_draft02_lookup(self):
        ...
        self.assertEqual(
            {"aki": "aki01", "foo": "bar01"},
            self.renewalinfo._draft02_lookup("serial", "aki01"),
        )
    ...
    def test_035_draft02_lookup(self):
        ...
        self.assertEqual(
            {"aki": "aki02", "foo": "bar02"},
            self.renewalinfo._draft02_lookup("serial", "aki02"),
        )

    def test_036_draft02_lookup(self):
        """test _draft02_lookup() failed with leading zero"""
        self.renewalinfo.dbstore.certificates_search.side_effect = [
            [],
            [{"foo": "bar01", "aki": "aki01"}, {"foo": "bar02", "aki": "aki02"}],
        ]
        self.assertEqual(
            {"aki": "aki02", "foo": "bar02"},
            self.renewalinfo._draft02_lookup("00serial", "aki02"),
        )
    ...
    def test_038_cert_table_update(self, mock_serial, mock_aki):
        """test _cert_table_update()"""
        self.renewalinfo.dbstore.certificates_search.side_effect = Exception(
            "certificates_search"
        )
        with self.assertLogs("test_a2c", level="INFO") as lcm:
            self.assertFalse(self.renewalinfo._cert_table_update())
    ...
    def test_041_serial_aki_get(self, mock_dec, mock_rec):
        ...
        self.assertEqual(
            ("616b69", "73657269616c"), self.renewalinfo._serial_aki_get("renewal.info")
        )
    ...
    def test_042_serial_aki_get(self, mock_dec, mock_rec):
        ...
        mock_rec.side_effect = ["foo1", "foo2"]
        self.assertEqual(
            (None, None), self.renewalinfo._serial_aki_get("ren.ewal.info")
        )
    ...
    def test_047_renewalinfo_generate(self):
        ...
        self.assertEqual(
            {
                "suggestedWindow": {
                    "start": "1970-01-01T00:14:25Z",
                    "end": "1970-01-01T00:16:40Z",
                }
            },
            self.renewalinfo._renewalinfo_generate(cert_dic),
        )
    ...
    def test_049_renewalinfo_generate(self, mock_uts):
        ...
        self.assertEqual(
            {
                "suggestedWindow": {
                    "start": "1971-09-29T00:00:00Z",
                    "end": "1972-09-28T00:00:00Z",
                }
            },
            self.renewalinfo._renewalinfo_generate(cert_dic),
        )
