# Removed all misplaced/indented test methods before imports. File now starts with import statements and class definitions only.
import unittest
...
class TestRenewalinfoConfig(unittest.TestCase):
    def test_default_values(self):
        ...
        self.assertEqual(config.renewaltreshold_pctg, 85.0)
...
class TestRenewalinfo(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_config = RenewalinfoConfig(renewal_force=True, renewaltreshold_pctg=90.0, retry_after_timeout=1234)
        patcher_db = patch("acme_srv.renewalinfo.DBstore", return_value=self.mock_dbstore)
        patcher_msg = patch("acme_srv.renewalinfo.Message", return_value=self.mock_message)
        patcher_err = patch("acme_srv.renewalinfo.error_dic_get", return_value={"malformed": "malf"})
        patcher_repo = patch("acme_srv.renewalinfo.RenewalinfoRepository", return_value=self.mock_repository)
        patcher_certid_hex = patch("acme_srv.renewalinfo.certid_hex_get", return_value=(None, "hex"))
        ...
        self.renewalinfo = Renewalinfo(debug=True, srv_name="srv", logger=self.mock_logger)
    ...
    def test_get_housekeeping_triggers_update(self):
        ...
        self.mock_repository.get_certificate_by_certid.return_value = {"expire_uts": 100000, "issue_uts": 90000}
        ...
        with patch("acme_srv.renewalinfo.string_sanitize", return_value="foo"):
            ...
            self.renewalinfo._get_renewalinfo_data = MagicMock(return_value={"suggestedWindow": {"start": "a", "end": "b"}})
    ...
    def test_get_returns_400_on_exception(self):
        self.mock_repository.get_housekeeping_param.return_value = True
        self.renewalinfo._get_renewalinfo_data = MagicMock(side_effect=Exception("fail"))
        with patch("acme_srv.renewalinfo.string_sanitize", return_value="foo"):
            result = self.renewalinfo.get("/acme/renewal-info/foo")
    ...
    def test_update_success(self):
        self.mock_message.check.return_value = (200, None, None, None, {"certid": "foo", "replaced": True}, None)
        self.mock_repository.get_certificate_by_certid.return_value = {"expire_uts": 100000, "issue_uts": 90000}
    ...
    def test_update_payload_missing(self):
        self.mock_message.check.return_value = (200, None, None, None, {"foo": "bar"}, None)
        with patch("acme_srv.renewalinfo.certid_hex_get", return_value=(None, "hex")):
            result = self.renewalinfo.update("content")
            self.assertEqual(result["code"], 400)

    def test_lookup_certificate_by_renewalinfo_dot(self):
        self.renewalinfo._extract_serial_and_aki_from_string = MagicMock(return_value=("serial", "aki"))
        self.renewalinfo._lookup_certificate_by_serial_and_aki = MagicMock(return_value={"foo": "bar"})
    ...
    def test_lookup_certificate_by_renewalinfo_nodot(self):
        with patch("acme_srv.renewalinfo.certid_hex_get", return_value=(None, "hex")):
            self.renewalinfo._lookup_certificate_by_certid = MagicMock(return_value={"foo": "bar"})
    ...
    def test_generate_renewalinfo_window_no_expire_uts(self):
        ...
        with patch("acme_srv.renewalinfo.uts_now", return_value=12345) as mock_uts_now:
            ...
            renewalinfo.config.renewaltreshold_pctg = 85.0
            result4 = renewalinfo._generate_renewalinfo_window(cert_dic)
    ...
    def test_load_configuration_all_valid(self):
        class DummyConfig:
            def getboolean(self, section, key, fallback=None): return True
            def get(self, section, key, fallback=None):
                if key == "renewaltreshold_pctg": return "99.9"
                if key == "retry_after_timeout": return "12345"
                return fallback
        ...
        with patch("acme_srv.renewalinfo.load_config", return_value=DummyConfig()):
            ...
            self.assertEqual(self.renewalinfo.config.renewaltreshold_pctg, 99.9)
    ...
    def test_load_configuration_defaults(self):
        ...
        with patch("acme_srv.renewalinfo.load_config", return_value=DummyConfig()):
            ...
            self.assertEqual(self.renewalinfo.config.renewaltreshold_pctg, 85.0)
            self.assertEqual(self.renewalinfo.config.retry_after_timeout, 86400)

    def test_load_configuration_renewal_force_error(self):
        class DummyConfig:
            def getboolean(self, section, key, fallback=None): raise Exception("failbool")
            def get(self, section, key, fallback=None): return fallback
            def __contains__(self, key): return True
        with patch("acme_srv.renewalinfo.load_config", return_value=DummyConfig()):
            self.renewalinfo.logger = MagicMock()
    ...
    def test_load_configuration_renewaltreshold_pctg_error(self):
        class DummyConfig:
            def getboolean(self, section, key, fallback=None): return False
            def get(self, section, key, fallback=None):
                if key == "renewaltreshold_pctg": raise Exception("failpctg")
                return fallback
            def __contains__(self, key): return True
        with patch("acme_srv.renewalinfo.load_config", return_value=DummyConfig()):
            self.renewalinfo.logger = MagicMock()
            self.renewalinfo.config = RenewalinfoConfig()
            self.renewalinfo._load_configuration()
            self.renewalinfo.logger.error.assert_any_call("renewaltreshold_pctg parsing error: %s", unittest.mock.ANY)
            self.assertEqual(self.renewalinfo.config.renewaltreshold_pctg, 85.0)
    ...
    def test_load_configuration_retry_after_timeout_error(self):
        class DummyConfig:
            def getboolean(self, section, key, fallback=None): return False
            def get(self, section, key, fallback=None):
                if key == "renewaltreshold_pctg": return "85.0"
                if key == "retry_after_timeout": raise Exception("failtimeout")
                return fallback
            def __contains__(self, key): return True
        with patch("acme_srv.renewalinfo.load_config", return_value=DummyConfig()):
            self.renewalinfo.logger = MagicMock()
            self.renewalinfo.config = RenewalinfoConfig()
            self.renewalinfo._load_configuration()
            self.renewalinfo.logger.error.assert_any_call("retry_after_timeout parsing error: %s", unittest.mock.ANY)
            self.assertEqual(self.renewalinfo.config.retry_after_timeout, 86400)
    ...
    def test_update_certificate_table_with_serial_and_aki_success(self):
        ...
        with patch("acme_srv.renewalinfo.cert_serial_get", return_value="serial1"), \
        ...
        mock_logger.debug.assert_any_call("Renewalinfo._update_certificate_table_with_serial_and_aki()")
        mock_logger.debug.assert_any_call("Renewalinfo._update_certificate_table_with_serial_and_aki(%s) - done", 1)
    ...
    def test_update_certificate_table_with_serial_and_aki_db_error(self):
        ...
        mock_logger.debug.assert_any_call("Renewalinfo._update_certificate_table_with_serial_and_aki(%s) - done", 0)
    ...
    def test_update_compat_success(self):
        ...
        renewalinfo.repository = MagicMock()
        renewalinfo.err_msg_dic = {"malformed": "malf"}
        renewalinfo.message.check.return_value = (200, None, None, None, {"certid": "foo", "replaced": True}, None)
        renewalinfo._lookup_certificate_by_renewalinfo = MagicMock(return_value={"foo": "bar"})
    ...
    def test_update_compat_payload_missing(self):
        ...
        renewalinfo.repository = MagicMock()
        renewalinfo.err_msg_dic = {"malformed": "malf"}
        renewalinfo.message.check.return_value = (200, None, None, None, {"foo": "bar"}, None)
        result = renewalinfo.update("content")
        self.assertEqual(result["code"], 400)
    ...
    def test_lookup_certificate_by_serial_and_aki_found(self):
        ...
        result = self.renewalinfo._lookup_certificate_by_serial_and_aki("serial123", "aki123")
        self.assertEqual(result, cert)
        self.renewalinfo.repository.get_certificates_by_serial.assert_called_once_with("serial123")
    ...
    def test_lookup_certificate_by_serial_and_aki_leading_zero(self):
        ...
        self.renewalinfo.repository.get_certificates_by_serial.side_effect = [[], [cert]]
        result = self.renewalinfo._lookup_certificate_by_serial_and_aki("0123", "aki456")
        self.assertEqual(result, cert)
        self.assertEqual(self.renewalinfo.repository.get_certificates_by_serial.call_count, 2)
    ...
    def test_lookup_certificate_by_serial_and_aki_not_found(self):
        ...
        self.renewalinfo.repository.get_certificates_by_serial.return_value = [{"aki": "other"}]
        result = self.renewalinfo._lookup_certificate_by_serial_and_aki("serial", "aki999")
        self.assertEqual(result, {})
    ...
    def test_get_renewalinfo_data(self):
        ...
        cert_dic = {"expire_uts": 100000, "issue_uts": 90000}
        renewalinfo_dic = {"suggestedWindow": {"start": "2025-01-01", "end": "2026-01-01"}}
        self.renewalinfo._lookup_certificate_by_renewalinfo = MagicMock(return_value=cert_dic)
        self.renewalinfo._generate_renewalinfo_window = MagicMock(return_value=renewalinfo_dic)
        result = self.renewalinfo._get_renewalinfo_data("foo.bar")
        self.renewalinfo._lookup_certificate_by_renewalinfo.assert_called_once_with("foo.bar")
