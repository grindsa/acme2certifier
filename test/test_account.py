...
class TestACMEHandler(unittest.TestCase):
    ...
    @patch('acme_srv.account.Account._tos_check')
    @patch('acme_srv.message.Message.check')
    def test_026_account_new(self, mock_mcheck, mock_tos, mock_contact, mock_aad, mock_nnonce, mock_eab, mock_retex):
        """ Account.new() successful for a existing account onlyreturnexisting """
        mock_mcheck.return_value = (200, None, None, 'protected', {'contact' : [u'mailto: foo@bar.com'], 'onlyreturnexisting': True}, None)
        mock_retex.return_value = (200, 'name', {'status': 'status', 'foo': 'bar'})
        mock_nnonce.return_value = 'new_nonce'
        message = {'foo' : 'bar'}
        e_result = {'code': 200, 'data': {'foo': 'bar', 'status': 'status'}, 'header': {'Location': 'http://tester.local/acme/acct/name', 'Replay-Nonce': 'new_nonce'}}
        self.assertEqual(e_result, self.account.new(message))
    ...
    def test_031_account_parse(self, mock_mcheck, mock_dea, mock_nnonce):
        ...
        mock_dea.return_value = (400, 'urn:ietf:params:acme:error:accountDoesNotExist', 'deletion failed')
        mock_nnonce.return_value = 'new_nonce'
        message = '{"foo" : "bar"}'
    ...
    @patch('acme_srv.account.Account._delete')
    @patch('acme_srv.message.Message.check')
    def test_034_account_parse(self, mock_mcheck, mock_del, mock_nnonce, mock_keychange):
    ...
    @patch('acme_srv.account.Account._contacts_update')
    @patch('acme_srv.message.Message.check')
    def test_036_account_parse(self, mock_mcheck, mock_contact_upd, mock_account_lookup, mock_nnonce, mock_info):
    ...
    @patch('acme_srv.account.Account._lookup')
    @patch('acme_srv.message.Message.check')
    def test_038_account_parse(self, mock_mcheck, mock_account_lookup, mock_nnonce, mock_info):
        """ test succ account parse for request for account info """
        mock_mcheck.return_value = (200, None, None, 'protected', {}, 'account_name')
        mock_nnonce.return_value = 'new_nonce'
    ...
    @patch('acme_srv.account.Account._contact_check')
    def test_046_account__contacts_update(self, mock_contact_chk,):
    ...
    @patch('acme_srv.account.Account._inner_jws_check')
    @patch('acme_srv.account.Account._lookup')
    def test_072_account__key_change_validate(self, mock_lup, mock_jws_chk, mock_pl_chk):
    ...
    @patch('acme_srv.account.Account._add')
    @patch('acme_srv.message.Message.check')
    def test_091_account_new(self, mock_mcheck, mock_aad, mock_nnonce, mock_tos, mock_eab):
    ...
    def test_095_account__onlyreturnexisting(self):
        ...
        self.assertIn('CRITICAL:test_a2c:acme2certifier database error in Account._account_lookup(): exc_acc_returnexit', lcm.output)
    ...
    def test_101_account__deactivate(self):
        ...
        with self.assertLogs('test_a2c', level='INFO') as lcm:
            self.assertEqual((400, 'urn:ietf:params:acme:error:accountDoesNotExist', 'deactivation failed'), self.account._deactivate('foo'))
        self.assertIn('CRITICAL:test_a2c:acme2certifier database error in Account._deactivate(): exc_deactivate', lcm.output)

    @patch('acme_srv.account.Account._contact_check')
    def test_102_account__contacts_update(self, mock_contact_chk,):
    ...
    def test_152__account_tune(self):
        ...
        input = {'jwk': 'jwk'}
        jwk = {'foo': 'bar'}
        self.assertEqual( {'key': {'foo': 'bar'}, 'status': 'valid'}, self.account._account_tune(input, jwk))

    def test_153__account_tune(self):
        """ test Account._account_tune()"""
        input = {'contact': '{"foo": "bar"}'}
        jwk = {'foo': 'bar'}
        self.assertEqual({'contact': {'foo': 'bar'}, 'status': 'valid'}, self.account._account_tune(input, jwk))
    ...
    def test_154__account_tune(self, mock_date):
        ...
        jwk = {'foo': 'bar'}
        mock_date.return_value = 'created_at'
        self.assertEqual({'created_at': 'created_at', 'status': 'valid'}, self.account._account_tune(input, jwk))
