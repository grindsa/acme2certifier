...
class TestOrderRepository(unittest.TestCase):
    ...
    def test_017_orders_invalid_search_success(self):
        ...
        self.assertEqual(
            self.repo.orders_invalid_search("expires", 0, [], "<="), ["order1"]
        )
...
class TestOrderClass(unittest.TestCase):
    def setUp(self):
        ...
        patcher_message = patch(
            "acme_srv.message.Message", return_value=self.mock_message
        )
        patcher_repo = patch(
    ...
    def test_044_create_from_content_error(self):
        ...
        with patch.object(
            ...
            with patch.object(
                self.order,
                "create_order",
                return_value=("someerror", "order", {}, "2026-01-01T00:00:00Z"),
            ):
                with patch.object(
                    self.order.message,
                    "prepare_response",
        ...
        with patch.object(self.order, "_name_get", return_value="order"):
            with patch.object(
                ...
                with patch.object(
                    ...
                    code, msg, detail, cert, order = self.order._parse_order_message(
                        {"url": "url"}, {}, None
                    )
    ...
    def test_051_add_order_and_authorizations_order_db_error(self):
        ...
        with patch.object(
            ...
            self.assertIn(
                "DEBUG:Order._add_order_and_authorizations() ended with malformed",
                log_contents,
            )
    ...
    def test_063_create_order_invalid_identifiers(self):
        ...
        account_name = "acct"
        with patch.object(
            self.order, "_check_identifiers_validity", return_value="rejectedidentifier"
        ) as mock_check, patch.object(
            self.order, "_add_order_and_authorizations", return_value=None
        ) as mock_add_order_authz:
            error, order_name, auth_dic, expires = self.order.create_order(
    ...
    def test_064_create_order_profile_invalid(self):
        ...
        account_name = "acct"
        with patch.object(
            self.order, "_check_identifiers_validity", return_value=None
        ) as mock_check, patch.object(
            self.order,
    ...
    def test_067_create_order_logging(self):
        ...
        with patch(
            ...
            order = Order(debug=True, server_name="test", logger=self.logger)
            with patch.object(
                order, "_check_identifiers_validity", return_value=None
            ), patch.object(
                order,
                "add_profile_to_order",
                return_value=(
    ...
    def test_083_name_get_with_slash(self):
        ...
        with patch(
            ...
            return_value={"path": "/acme/order/ord123/extra"},
        ):
            order = Order(debug=True, server_name="test", logger=self.logger)
    ...
    def test_114_get_authorization_list_all_paths(self):
        ...
        with patch(
            ...
            mock_repo = MagicMock()
            mock_repo.authorization_lookup.return_value = [
                {"name": "auth1", "status__name": "valid"}
            ]
            mock_repo_class.return_value = mock_repo
            order = Order(debug=True, server_name="test", logger=self.logger)
    ...
    def test_115_update_validity_list_all_paths(self):
    ...
    def test_116_get_order_details_all_paths(self):
    ...
    def test_117_invalidate_expired_orders_all_paths(self):
        # Covers: success, db error, logging
        self.log_stream.truncate(0)
        self.log_stream.seek(0)
        self.logger.setLevel(logging.DEBUG)
    ...
    def test_118_create_from_content_all_paths(self):
        ...
        with patch(
            ...
            order.create_order = MagicMock(
                return_value=(None, "order1", {}, 1234567890)
            )
    ...
    def test_119_process_order_request_all_paths(self):
    ...
    def test_120_check_identifiers_validity_all_paths(self):
        ...
        self.assertIn(
            "DEBUG:Order._check_identifiers_validity() done with", log_contents
        )
    ...
    def test_129_process_order_request_no_url(self):
        ...
        with patch.object(self.order.logger, "debug") as mock_debug:
            ...
            found = any(
                call_args[0] == expected_call for call_args in mock_debug.call_args_list
            )
            self.assertTrue(
                found, f"Expected log call not found in: {mock_debug.call_args_list}"
            )
