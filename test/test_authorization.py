...
class TestAuthorizationRepository(unittest.TestCase):
    ...
    def test_0010_find_authorization_by_name_success(self):
        ...
        self.mock_dbstore.authorization_lookup.assert_called_once_with("name", "test_authz")
    ...
    def test_0011_find_authorization_by_name_with_field_list(self):
        ...
        self.assertEqual(result, expected_result)
        self.mock_dbstore.authorization_lookup.assert_called_once_with("name", "test_authz", field_list)
    ...
    def test_0014_find_authorization_by_name_database_error(self):
        ...
        self.mock_dbstore.authorization_lookup.side_effect = Exception("Database connection failed")

        with self.assertRaises(AuthorizationError) as context:
            self.repository.find_authorization_by_name("test_authz")

        self.assertIn("Failed to find authorization 'test_authz': Database connection failed", str(context.exception))
    ...
    def test_0015_update_authorization_expiry_success(self):
        ...
        self.repository.update_authorization_expiry("test_authz", "new_token", 1234567890)
    ...
    def test_0016_update_authorization_expiry_database_error(self):
        ...
        self.assertIn("Failed to update authorization 'test_authz': Update failed", str(context.exception))
    ...
    def test_0018_search_expired_authorizations_database_error(self):
        ...
        self.mock_dbstore.authorizations_expired_search.side_effect = Exception("Search failed")

        with self.assertRaises(AuthorizationError) as context:
            self.repository.search_expired_authorizations(1234567890, ["name"])

        self.assertIn("Failed to search expired authorizations: Search failed", str(context.exception))
        self.mock_logger.critical.assert_called_once()
    ...
    def test_0020_mark_authorization_as_expired_database_error(self):
        ...
        self.assertIn("Failed to expire authorization 'test_authz': Expire failed", str(context.exception))
        self.mock_logger.critical.assert_called_once()
...
class TestAuthorizationBusinessLogic(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_logger = Mock()
        self.business_logic = AuthorizationBusinessLogic(self.config, self.mock_repository, self.mock_logger)
    ...
    def test_0022_extract_authorization_name_from_url(self, mock_sanitize):
        ...
        result = self.business_logic.extract_authorization_name_from_url(url, server_name)
    ...
    def test_0024_generate_authorization_token_and_expiry(self, mock_uts_now, mock_generate_string):
    ...
    def test_0025_enrich_authorization_with_identifier_info_empty(self):
        ...
        result, is_tnauth = self.business_logic.enrich_authorization_with_identifier_info(None)
    ...
    def test_0026_enrich_authorization_with_identifier_info_dict(self):
        ...
        result, is_tnauth = self.business_logic.enrich_authorization_with_identifier_info(auth_info)
...
class TestChallengeSetManager(unittest.TestCase):
    ...
    def test_0040_get_challenge_set_for_authorization_success(self, mock_challenge_class):
        ...
        mock_challenge_class.return_value.__enter__.return_value = mock_challenge_instance
    ...
    @patch('acme_srv.authorization.Challenge')
    def test_0041_get_challenge_set_for_authorization_with_none_values(self, mock_challenge_class):
...
class TestAuthorization(unittest.TestCase):
    ...
    def setUp(self):
        self.mock_logger = Mock()
        self.patcher_dbstore = patch('acme_srv.authorization.DBstore')
        self.patcher_message = patch('acme_srv.authorization.Message')
        self.patcher_nonce = patch('acme_srv.authorization.Nonce')

        self.mock_dbstore_class = self.patcher_dbstore.start()
        self.mock_message_class = self.patcher_message.start()
        self.mock_nonce_class = self.patcher_nonce.start()

        self.mock_dbstore = Mock()
        self.mock_message = Mock()
        self.mock_nonce = Mock()

        self.mock_dbstore_class.return_value = self.mock_dbstore
        self.mock_message_class.return_value = self.mock_message
        self.mock_nonce_class.return_value = self.mock_nonce

    def tearDown(self):
        self.patcher_dbstore.stop()
        self.patcher_message.stop()
        self.patcher_nonce.stop()
    ...
    def test_0044_authorization_context_manager_enter(self, mock_load_config):
    ...
    @patch('acme_srv.authorization.load_config')
    def test_0046_load_configuration_success(self, mock_load_config):
    ...
    def test_0047_load_configuration_invalid_validity(self, mock_load_config):
        ...
        self.assertIn("Invalid validity parameter: invalid_number", str(context.exception))

    @patch('acme_srv.authorization.load_config')
    def test_0048_load_configuration_empty_config(self, mock_load_config):
    ...
    def test_0051_authz_info_backward_compatibility(self):
        ...
        with patch.object(authorization, 'get_authorization_details') as mock_get_details:
    ...
    def test_0052_get_authorization_details_not_found(self):
        ...
        result = authorization.get_authorization_details("http://example.com/authz/test")
    ...
    def test_0053_get_authorization_details_success_minimal(self, mock_uts_to_date):
        ...
        mock_business_logic.extract_authorization_name_from_url.return_value = "test_authz"
        mock_business_logic.generate_authorization_token_and_expiry.return_value = ("token", 1234567890)
        mock_business_logic.extract_identifier_info_for_challenge.return_value = (None, None)
        ...
        self.assertEqual(result, expected)
        mock_repository.update_authorization_expiry.assert_called_once_with("test_authz", "token", 1234567890)

    @patch('acme_srv.authorization.uts_to_date_utc')
    def test_0054_get_authorization_details_success_with_details(self, mock_uts_to_date):
        ...
        mock_business_logic.enrich_authorization_with_identifier_info.return_value = (
            {"status": "valid", "identifier": {"type": "dns", "value": "example.com"}}, False
        )
        mock_business_logic.extract_identifier_info_for_challenge.return_value = ("dns", "example.com")
        mock_challenge_manager.get_challenge_set_for_authorization.return_value = [{"type": "http-01"}]
    ...
    def test_0055_get_authorization_details_challenge_error(self):
        ...
        mock_business_logic.generate_authorization_token_and_expiry.return_value = ("token", 1234567890)
        mock_business_logic.extract_identifier_info_for_challenge.return_value = ("dns", "example.com")
        mock_challenge_manager.get_challenge_set_for_authorization.side_effect = Exception("Challenge failed")
    ...
    def test_0056_expire_invalid_authorizations_default_timestamp(self, mock_uts_now):
        ...
        mock_repository.mark_authorization_as_expired.assert_called_once_with("expired_authz")
    ...
    def test_0057_expire_invalid_authorizations_custom_timestamp(self):
        ...
        field_list, output_list = authorization.expire_invalid_authorizations(timestamp=1000000000)

        self.assertEqual(len(output_list), 1)
        mock_repository.search_expired_authorizations.assert_called_with(1000000000, field_list)
    ...
    def test_0058_expire_invalid_authorizations_search_error(self):
        ...
        # Replace components with mocks
        mock_repository = Mock()
        mock_repository.search_expired_authorizations.side_effect = AuthorizationError("Search failed")
        authorization.repository = mock_repository
    ...
    def test_0059_expire_invalid_authorizations_not_eligible(self):
        ...
        mock_repository.search_expired_authorizations.return_value = [not_eligible_authz]
    ...
    def test_0060_expire_invalid_authorizations_expire_error(self):
        ...
        mock_repository.search_expired_authorizations.return_value = [expired_authz]
        mock_business_logic.is_authorization_eligible_for_expiry.return_value = True
        mock_repository.mark_authorization_as_expired.side_effect = AuthorizationError("Expire failed")
        ...
        self.assertEqual(len(output_list), 1)  # Authorization is added before expiration fails
        # Check that warning was called with the right pattern
        self.assertTrue(self.mock_logger.warning.called)
        call_args = self.mock_logger.warning.call_args[0]
    ...
    def test_0065_handle_post_request_success_with_expiry_check(self):
        ...
        # Mock message check
        self.mock_message.check.return_value = (200, "OK", "", {"url": "http://example.com/authz/test"}, {}, "account")
    ...
    def test_0067_handle_post_request_invalidate_error(self):
        ...
        self.assertEqual(result, {"final": "response"})
        # Check that warning was called with the right pattern - actual args may vary
        self.assertTrue(self.mock_logger.warning.called)
    ...
    def test_0068_handle_post_request_message_check_failure(self):
        ...
        self.mock_message.check.return_value = (400, "Bad Request", "Invalid message", {}, {}, "")
    ...
    def test_0069_handle_post_request_missing_url(self):
        ...
        self.mock_message.check.return_value = (200, "OK", "", {}, {}, "account")  # No "url" in protected
    ...
    def test_0074_invalidate_backward_compatibility(self):
        ...
        with patch.object(authorization, 'expire_invalid_authorizations') as mock_expire:
