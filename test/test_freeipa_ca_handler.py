...
class TestCAhandler(unittest.TestCase):
    ...
    def test_003_config_load_fqdn_fallback(self):
        # Simulate missing fqdn in DEFAULT section, should log and fallback to CONFIG_SECTION
        with patch('examples.ca_handler.freeipa_ca_handler.load_config') as mock_load_config:
            config = MagicMock()
            def get_side_effect(section, key, fallback=None):
                if section == self.handler.CONFIG_DEFAULT_SECTION and key == 'fqdn':
                    return ''
    ...
    def test_003_login_http_error_testcase(self):
        ...
        self.handler.session = MagicMock()
        self.handler.session.post.return_value.raise_for_status.side_effect = requests.exceptions.HTTPError('HTTP error')
        self.handler.api_host = 'https://ipa.example.com'
        self.handler.api_user = 'admin'
        self.handler.api_password = 'secret'
        self.handler.ca_bundle = True
    ...
    def test_030_login_connection_error_testcase(self):
        ...
        self.handler.session = MagicMock()
        self.handler.session.post.return_value.raise_for_status.side_effect = requests.exceptions.ConnectionError('Connection error')
        self.handler.api_host = 'https://ipa.example.com'
        self.handler.api_user = 'admin'
        self.handler.api_password = 'secret'
        self.handler.ca_bundle = True
    ...
    def test_031_login_timeout_error_testcase(self):
        ...
        self.handler.session = MagicMock()
        self.handler.session.post.return_value.raise_for_status.side_effect = requests.exceptions.Timeout('Timeout error')
        self.handler.api_host = 'https://ipa.example.com'
        self.handler.api_user = 'admin'
        self.handler.api_password = 'secret'
        self.handler.ca_bundle = True
    ...
    def test_032_login_request_exception_testcase(self):
        ...
        self.handler.session = MagicMock()
        self.handler.session.post.return_value.raise_for_status.side_effect = requests.exceptions.RequestException('Request exception')
        self.handler.api_host = 'https://ipa.example.com'
        self.handler.api_user = 'admin'
        self.handler.api_password = 'secret'
        self.handler.ca_bundle = True
    ...
    def test_007_host_add_principal_success_logs_debug(self):
        ...
        with patch.object(self.handler, '_rpc_post', return_value={}), \
             patch.object(self.handler, 'logger') as mock_logger:
            self.handler._host_add_principal('host1', 'fqdn1')
            mock_logger.debug.assert_called_with('Host principal %s added to host %s successfully', 'fqdn1', 'host1')
    ...
    def test_007_host_add_success_logs_info(self):
        ...
        with patch.object(self.handler, '_rpc_post', return_value={}), \
             patch.object(self.handler, 'logger') as mock_logger:
            self.handler._host_add('host1')
            mock_logger.info.assert_called_with('Host %s added successfully to freeIPA', 'host1')
    ...
    def test_0010_host_add_invalid_hostname(self):
        ...
        with patch.object(self.handler, 'logger') as mock_logger:
            ...
            mock_logger.error.assert_called_with('Invalid hostname provided to _host_add: %s', None)
        # Should log error and return for invalid hostname (not a string)
        with patch.object(self.handler, 'logger') as mock_logger:
            self.handler._host_add(12345)
            mock_logger.error.assert_called_with('Invalid hostname provided to _host_add: %s', 12345)
    ...
    def test_008_host_add_principal_invalid_fqdn(self):
        ...
        with patch.object(self.handler, 'logger') as mock_logger:
            ...
            mock_logger.error.assert_called_with('Invalid fqdn provided to _host_add_principal: %s', None)
        with patch.object(self.handler, 'logger') as mock_logger:
            self.handler._host_add_principal(None, 'fqdn')
            mock_logger.error.assert_called_with('Invalid hostname provided to _host_add_principal: %s', None)
        with patch.object(self.handler, 'logger') as mock_logger:
            self.handler._host_add_principal('host', 12345)
            mock_logger.error.assert_called_with('Invalid fqdn provided to _host_add_principal: %s', 12345)

    def test_009_host_add_principal_error_and_success(self):
        # Should log error if _rpc_post returns error, else log success
        self.handler.api_version = '2.257'
        with patch.object(self.handler, '_rpc_post', return_value={'error': 'fail'}), \
                patch.object(self.handler, 'logger') as mock_logger:
            self.handler._host_add_principal('host', 'fqdn')
            mock_logger.error.assert_called_with('Failed to add host principal %s for host %s: %s', 'fqdn', 'host', 'fail')
        with patch.object(self.handler, '_rpc_post', return_value={}), \
                patch.object(self.handler, 'logger') as mock_logger:
            self.handler._host_add_principal('host', 'fqdn')
            mock_logger.debug.assert_called_with('Host principal %s added to host %s successfully', 'fqdn', 'host')
    ...
    def test_007_host_search_invalid_hostname(self):
        ...
        with patch.object(self.handler, 'logger') as mock_logger:
            ...
            mock_logger.error.assert_called_with('Invalid hostname provided to _host_search: %s', None)
        with patch.object(self.handler, 'logger') as mock_logger:
            result = self.handler._host_search(12345)
            self.assertEqual(result, {})
            mock_logger.error.assert_called_with('Invalid hostname provided to _host_search: %s', 12345)

    def test_008_parse_csr_valid_testcase(self):
        with patch('examples.ca_handler.freeipa_ca_handler.csr_cn_get', return_value='cn'), \
             patch('examples.ca_handler.freeipa_ca_handler.csr_san_get', return_value=['DNS:foo', 'DNS:bar']):
    ...
    def test_009_parse_csr_no_cn_testcase(self):
        with patch('examples.ca_handler.freeipa_ca_handler.csr_cn_get', return_value=None), \
             patch('examples.ca_handler.freeipa_ca_handler.csr_san_get', return_value=['DNS:foo']):
            cn, san_list = self.handler._parse_csr('dummycsr')
    ...
    def test_010_parse_csr_invalid_input(self):
        ...
        with patch.object(self.handler, 'logger') as mock_logger:
            ...
            mock_logger.error.assert_called_with('Invalid CSR provided to _parse_csr: %s', None)
        with patch.object(self.handler, 'logger') as mock_logger:
            result = self.handler._parse_csr(12345)
            self.assertEqual(result, (None, []))
            mock_logger.error.assert_called_with('Invalid CSR provided to _parse_csr: %s', 12345)
    ...
    def test_013_rpc_post_connection_error(self):
        ...
        self.handler.session.post.side_effect = requests.exceptions.ConnectionError('fail')
        result = self.handler._rpc_post({'foo': 'bar'})
        self.assertIn('error', result)
    ...
    def test_015_rpc_post_request_exception(self):
        ...
        self.handler.session.post.side_effect = requests.exceptions.RequestException('fail')
        result = self.handler._rpc_post({'foo': 'bar'})
        self.assertIn('error', result)
    ...
    def test_012_extract_api_version_testcase(self):
        self.handler._ipa_ping = MagicMock(return_value={'result': {'summary': 'API version 2.257'}})
    ...
    def test_013_cert_chain_to_pem_testcase(self):
        with patch('examples.ca_handler.freeipa_ca_handler.b64_decode', return_value=b'bytes'), \
             patch('examples.ca_handler.freeipa_ca_handler.cert_der2pem', return_value=b'PEM'):
    ...
    def test_015_enroll_success_testcase(self):
        self.handler._rpc_post = MagicMock(return_value={'result': {'result': {'certificate': 'cert', 'certificate_chain': []}}})
    ...
    def test_300_enroll_unexpected_result_structure(self):
        ...
        self.handler._rpc_post = MagicMock(return_value={'result': {'result': 'notadict'}})
        with patch.object(self.handler, 'logger') as mock_logger:
            error, cert_bundle, cert_raw = self.handler._enroll('host', 'csr')
    ...
    def test_301_enroll_exception_in_result_extraction(self):
        ...
        self.handler._rpc_post = MagicMock(return_value={'result': {'result': {'certificate': 'cert', 'certificate_chain': None}}})
        with patch.object(self.handler, '_cert_chain_to_pem', side_effect=Exception('fail')), \
             patch.object(self.handler, 'logger') as mock_logger:
            error, cert_bundle, cert_raw = self.handler._enroll('host', 'csr')
    ...
    def test_018_handler_check_testcase(self):
        with patch('examples.ca_handler.freeipa_ca_handler.handler_config_check', return_value=None):
            error = self.handler.handler_check()
            self.assertIsNone(error)

    def test_019_poll_testcase(self):
        error, cert_bundle, cert_raw, poll_identifier, rejected = self.handler.poll('cert', 'pollid', 'csr')
        self.assertIsNone(error)
        self.assertIsNone(cert_bundle)
        self.assertIsNone(cert_raw)
    ...
    def test_040_host_add_managedby_success_testcase(self):
        ...
        self.logger.debug.assert_any_call('Host %s added managed by %s successfully', 'host1', 'ipa.example.com')
    ...
    def test_100_revoke_public_error(self):
        ...
        with patch('examples.ca_handler.freeipa_ca_handler.cert_serial_get', return_value='serial'):
    ...
    def test_101_revoke_public_success(self):
        ...
        with patch('examples.ca_handler.freeipa_ca_handler.cert_serial_get', return_value='serial'):
            self.handler._revoke = MagicMock(return_value=(0, 'Certificate revoked successfully', None))
            code, message, detail = self.handler.revoke('dummycert')
            self.handler._revoke.assert_called_once_with('serial')
            self.assertEqual(code, 0)
            self.assertEqual(message, 'Certificate revoked successfully')
    ...
    def test_102_revoke_no_serial(self):
        ...
        with patch('examples.ca_handler.freeipa_ca_handler.cert_serial_get', return_value=None):
    ...
    def test_200_enroll_eab_profile_error(self):
        ...
        with patch('examples.ca_handler.freeipa_ca_handler.eab_profile_header_info_check', return_value='eab_error'):
    ...
    def test_201_enroll_ensure_host_error(self):
        ...
        with patch('examples.ca_handler.freeipa_ca_handler.eab_profile_header_info_check', return_value=None), \
             patch.object(self.handler, '_parse_csr', return_value=('host', ['alias1', 'alias2'])), \
             patch.object(self.handler, '_ensure_host_and_principals', return_value='host_error'):
            result = self.handler.enroll('dummycsr')
    ...
    def test_202_enroll_enroll_error(self):
        ...
        with patch('examples.ca_handler.freeipa_ca_handler.eab_profile_header_info_check', return_value=None), \
             patch.object(self.handler, '_parse_csr', return_value=('host', ['alias1', 'alias2'])), \
             patch.object(self.handler, '_ensure_host_and_principals', return_value=None), \
             patch('examples.ca_handler.freeipa_ca_handler.build_pem_file', return_value='pemcsr'), \
             patch.object(self.handler, '_enroll', return_value=('enroll_error', None, None)):
            result = self.handler.enroll('dummycsr')
    ...
    def test_400_cert_chain_to_pem_skips_missing_base64(self):
        ...
        with patch('examples.ca_handler.freeipa_ca_handler.b64_decode') as mock_b64_decode, \
             patch('examples.ca_handler.freeipa_ca_handler.cert_der2pem') as mock_cert_der2pem:
