...
class Renewalinfo(object):
    ...
    def _config_load(self):
        ...
        if "Renewalinfo" in config_dic:
            ...
            self.renewal_force = config_dic.getboolean(
                "Renewalinfo", "renewal_force", fallback=False
            )
            try:
                self.renewaltreshold_pctg = float(
                    config_dic.get(
                        "Renewalinfo",
                        "renewaltreshold_pctg",
                        fallback=self.renewaltreshold_pctg,
                    )
                )
            ...
            try:
                self.retry_after_timeout = int(
                    config_dic.get(
                        "Renewalinfo",
                        "retry_after_timeout",
                        fallback=self.retry_after_timeout,
                    )
                )
            except Exception as err_:
                self.logger.error(
    ...
    def _cert_dic_lookup(self, renewalinfo_string: str) -> Dict[str, str]:
        ...
        self.logger.debug(
            "Renewalinfo._cert_dic_lookup(%s) - ended with: %s",
            renewalinfo_string,
            bool(cert_dic),
        )
        return cert_dic
    ...
    def _draft01_lookup(self, certid_hex: str) -> Dict[str, str]:
        ...
        try:
            result_dic = self.dbstore.certificate_lookup(
                "renewal_info",
                certid_hex,
                (
                    "id",
    ...
    def _draft02_lookup(self, serial: str, aki: str) -> Dict[str, str]:
        ...
        cert_dic = {}
        try:
            cert_list = self.dbstore.certificates_search(
                "serial",
                serial,
                operant="is",
                vlist=[
        ...
        self.logger.debug(
            "Renewalinfo._draft02_lookup() ended with: %s", bool(cert_dic)
        )
        return cert_dic
    ...
    def _renewalinfo_get(self, renewalinfo_string: str) -> Dict[str, str]:
        ...
        self.logger.debug(
            "Renewalinfo._renewalinfo_get() ended with: %s", rewalinfo_dic
        )
    ...
    def _renewalinfo_string_get(self, url: str) -> str:
        ...
        self.logger.debug(
            "Renewalinfo.renewal_string_get() - renewalinfo_string: %s",
            renewalinfo_string,
        )
    ...
    def _serial_aki_get(self, renewalinfo_string: str) -> (str, str):
        ...
        self.logger.debug(
            "Renewalinfo._serial_aki_get() - serial: %s, aki: %s", serial, aki
        )
        return (serial, aki)
    ...
    def get(self, url: str) -> Dict[str, str]:
        ...
        if rewalinfo_dic:
            ...
            response_dic["header"] = {
                "Retry-After": f"{self.retry_after_timeout}".format()
            }
        else:
