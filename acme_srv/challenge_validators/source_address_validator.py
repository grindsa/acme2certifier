...
class SourceAddressValidator(ChallengeValidator):
    ...
    def __init__(self, logger, forward_check: bool = False, reverse_check: bool = False):
    ...
    def perform_validation(self, context: ChallengeContext) -> ValidationResult:
        ...
        if self.forward_check:
            self.logger.debug("SourceAddressValidator.perform_validation(): Performing forward address check")
            ...
            if not forward_result.get("forward_check_passed", False):
                self.logger.debug("SourceAddressValidator.perform_validation(): Forward address check failed")
                return ValidationResult(
                    success=False,
                    invalid=True,
        ...
        if self.reverse_check:
            self.logger.debug("SourceAddressValidator.perform_validation(): Performing reverse address check")
            ...
            if not reverse_result.get("reverse_check_passed", False):
                self.logger.debug("SourceAddressValidator.perform_validation(): Reverse address check failed")
                return ValidationResult(
                    success=False,
                    invalid=True,
    ...
    def _perform_forward_check(self, domain: str, source_address: str, dns_servers: List[str]) -> Dict[str, Any]:
        """Perform forward DNS lookup to verify source address."""
        self.logger.debug("SourceAddressValidator._perform_forward_check(): Performing forward address check: %s -> %s", domain, source_address)

        try:
            from acme_srv.helper import fqdn_resolve

            # Resolve the domain to IP addresses
            resolved_ips, _invalid = fqdn_resolve(logger=self.logger, host=domain, dnssrv=dns_servers, catch_all=True)

            # Check if source address matches any resolved IP
            forward_check_passed = source_address in resolved_ips
            self.logger.debug("SourceAddressValidator._perform_forward_check(): Forward check %s for %s", "passed" if forward_check_passed else "failed", domain)
            return {
    ...
    def _perform_reverse_check(self, domain: str, source_address: str, dns_servers: List[str]) -> Dict[str, Any]:
        """Perform reverse DNS lookup to verify domain ownership."""
        self.logger.debug("SourceAddressValidator._perform_reverse_check(): Performing reverse address check: %s -> %s", source_address, domain)
        try:
            from acme_srv.helper import ptr_resolve

            # Perform reverse lookup on source address
            reverse_domains = ptr_resolve(self.logger, source_address, dnssrv=dns_servers)
            ...
            self.logger.debug("SourceAddressValidator._perform_reverse_check(): Reverse check %s for %s", "passed" if reverse_check_passed else "failed", domain)
            return {
