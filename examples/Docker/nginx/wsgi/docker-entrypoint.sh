...
if [[ "$DB_HANDLER" == "django" ]]; then
    echo "WARNING: handler resolves to django but this is a WSGI image; nginx/uWSGI entry and packages stay WSGI. Selection only — use a django image for a full Django stack." >> /proc/1/fd/1
fi
