...
# uWSGI ini env lines: quote values so $ is not interpreted. Reject @( / %(
# placeholders (uWSGI opens them as files / interpolates even inside quotes).
a2c_uwsgi_env_set() {
...
if [[ "${EL_MAJOR}" == "8" && "${FLAVOR_PKG}" == "$(pkg_name)-python3" ]]; then
  echo
