...
class CAhandler(object):
    ...
    def certs_bundle(self, uts):
        """ create needed cert(bundle) """
        self.logger.debug('CAhandler.certs_bundle()')
    ...
    def csr_san_get(self, csr):
        ...
        self.logger.debug('CAhandler.csr_san_get() ended with: {0}'.format(sans))
        return sans
    ...
    def enroll(self, csr):
        ...
        if self.openssl_bin:
            ...
            try:
                san_list = self.csr_san_get(csr)
            except BaseException:
                san_list = []

            if subject and pubkey and san_list:
                # dump public key
                self.pubkey_save(uts, pubkey)

                # build openssl command and run it
                openssl_cmd = self.opensslcmd_build(uts, subject, san_list)
                subprocess.call(openssl_cmd)

                # generate certificates we need to return
                if os.path.isfile('{0}/{1}_cert.pem'.format(self.tmp_dir, uts)):
                    (cert_bundle, cert_raw) = self.certs_bundle(uts)
                else:
                    error = 'Enrollment failed'

                # delete temporary files
                self.tmp_files_delete(uts)
            else:
                error = 'CSR invalid'
        else:
    ...
    def opensslcmd_build(self, uts, subject, san_list):
        """ build openssl command """
        self.logger.debug('CAhandler.opensslcmd_build()')
    ...
    def pubkey_save(self, uts, pubkey):
        """ save public key to file """
        self.logger.debug('CAhandler.pubkey_save()')
        with open('{0}/{1}_pubkey.pem'.format(self.tmp_dir, str(uts)), 'wb') as fso:
            fso.write(pubkey)
        self.logger.debug('CAhandler.pubkey_save() ended')
    ...
    def tmp_files_delete(self, uts):
        """ delete temp files """
        self.logger.debug('CAhandler.tmp_files_delete({0})'.format(uts))

        if os.path.isfile('{0}/{1}_cert.pem'.format(self.tmp_dir, uts)):
            os.remove('{0}/{1}_cert.pem'.format(self.tmp_dir, uts))
            self.logger.debug('CAhandler.tmp_files_delete() cert')
        if os.path.isfile('{0}/{1}_pubkey.pem'.format(self.tmp_dir, uts)):
            os.remove('{0}/{1}_pubkey.pem'.format(self.tmp_dir, uts))
            self.logger.debug('CAhandler.tmp_files_delete() pubkey')
        if os.path.isfile('{0}/{1}_capubs.pem'.format(self.tmp_dir, uts)):
            os.remove('{0}/{1}_capubs.pem'.format(self.tmp_dir, uts))
            self.logger.debug('CAhandler.tmp_files_delete() capub')
