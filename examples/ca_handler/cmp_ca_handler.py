...
class CAhandler(object):
    ...
    def _config_cmpparameter_load(self, ele: str, config_dic: Dict[str, str]):
        ...
        if ele == 'cmp_openssl_bin':
            ...
            try:
                ...
                self.logger.error('CAhandler._config_load() could not load cmp_ref:%s', err)
        elif ele == 'cmp_secret_variable':
            try:
                self.secret = os.environ[config_dic['CAhandler']['cmp_secret_variable']]
            except Exception as err:
                self.logger.error('CAhandler._config_load() could not load cmp_secret_variable:%s', err)
    ...
    def _opensslcmd_build(self) -> List[str]:
        ...
        self.logger.debug('CAhandler._opensslcmd_build() ended with: %s', ' '.join(cmd_list))
        return cmd_list
