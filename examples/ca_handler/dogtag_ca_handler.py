    ...
    for policy in data.get("ProfilePolicySet", [])[0].get("policies", []):
        for attr in policy.get("def", {}).get("attributes", []):
            if attr.get("name") == "notBefore":
                ...
                logger.debug("update_validity_attributes(): Setting notBefore attribute value (redacted)")
                attr["Value"] = notbefore
            elif attr.get("name") == "notAfter":
                # Do not log actual notAfter value (could be sensitive)
                logger.debug("update_validity_attributes(): Setting notAfter attribute value (redacted)")
                attr["Value"] = notafter
...
def approve_profile_get(logger, nonce: str = None, request_id: str = None):
    ...
    Args:
        logger: Logger instance for debug output.
        nonce (str, optional): Nonce value for the request.
        request_id (str, optional): Certificate request ID.
    Returns:
...
class CAhandler(object):
    ...
    def _api_version_get(self):
        """get API version from CA"""
    ...
    def _get_approval_nonce(self, request_id: str) -> Optional[str]:
        ...
        if 'requestStatus' in response:
            ...
            self.logger.debug("CAHandler._certrequest_approve() - Certificate request %s status before approval attempt", request_id)
        ...
        self.logger.debug("CAhandler._get_approval_nonce() ended with error: %s", error if error else "None")
        return error, nonce

    def _certrequest_approve(self, request_id: str) -> Tuple[str, str, str]:
        """
        Approve cert request with given request_id.
        Returns:
            Tuple[str, str, str]: error message (or None), cert_bundle, cert_raw
        """
        ...
        if error:
            self.logger.error("Cannot approve certificate request %s due to error: %s", request_id, error)
            return (error, cert_bundle, cert_raw)

        # approve request
        data = approve_profile_get(self.logger, nonce, request_id)
        code, response = self._api_post(f"{self.REST_AGENT_CERTREQUESTS}{request_id}/approve", data)

        # get new status and certificate data
        code, response = self._api_get(f"{self.REST_CERTREQUESTS}/{request_id}")

        if 'requestStatus' in response:
            # lookup serial numner
            if 'certId' in response:
                # Do not log certId (sensitive)
                self.logger.debug("CACertificate request %s approved successfully (certId redacted)", request_id)

                # fetch certificates based on
                code, response = self._api_get(f'{self.REST_CERTS}{response['certId']}')

                # self._parse_cert(response.get('Encoded'))

                # convert pkcs7 chain to pem
                if code == 200:
                    if 'Encoded' in response and response['Encoded']:
                        cert_raw = b64_encode(self.logger, cert_pem2der(response.get('Encoded')))
                    else:
                        # Do not log raw certificate data
                        self.logger.error("Certificate request approved but failed to fetch raw certificate data. Status code: %s", code)
                    ...
                    if 'PKCS7CertChain' in response and response['PKCS7CertChain']:
                        cert_bundle = pkcs7_to_pem(self.logger, b64_decode(self.logger, response.get('PKCS7CertChain')))
                    else:
                        # Do not log certificate chain data
                        self.logger.error("Certificate request approved but failed to fetch certificate chain data. Status code: %s", code)
                ...
                error = "Certificate request approved but certId is missing in the response, cannot fetch certificate data."

        else:
            self.logger.error("Failed to get certificate request status after approval attempt. Status code: %s, Response: %s", code, response)
            error = "Failed to approve certificate request or retrieve certificate data after approval attempt."

        self.logger.debug("CAhandler._certrequest_approve() ended with error: %s", error)
        return (error, cert_bundle, cert_raw)


    def _parse_cert(self, pem_data):
        """
    ...
    def _certrequest_send(self, csr: str = None) -> Tuple[str, str]:
        """
        Send a certificate signing request (CSR) to the CA.
        Args:
            csr (str, optional): PEM-encoded CSR string.
        Returns:
            Tuple[str, str]: Request ID and request status.
        """
        ...
        if code == 200:
            if isinstance(response, dict) and "entries" in response:
                ...
                if response["entries"] and "requestId" in response["entries"][0] and "requestStatus" in response["entries"][0]:
                    request_id = response["entries"][0]["requestId"]
                    request_status = response["entries"][0]["requestStatus"]
                    self.logger.info("Request ID: %s, Request Status: %s", request_id, request_status)
        ...
        self.logger.debug("CAhandler._certrequest_send() ended with code: %s, request_id: %s, request_status: %s", code, request_id, request_status)
        return request_id, request_status

    def _config_passphrase_load(self, config_dic: dict):
        """
        ...
        if self.CONFIG_SECTION in config_dic and isinstance(config_dic[self.CONFIG_SECTION], dict):
            section = config_dic[self.CONFIG_SECTION]
            if (
                "cert_passphrase_variable" in section
                or "cert_passphrase" in section
            ):
                if "cert_passphrase_variable" in section:
                    var_name = section["cert_passphrase_variable"]
                    try:
                        import os
                ...
                if "cert_passphrase" in section:
                    ...
                    self.cert_passphrase = section["cert_passphrase"]
    ...
    def _config_load(self):
        ...
        self.certrequest_approve = config_dic.getboolean(
            self.CONFIG_SECTION, "certrequest_approve", fallback=self.certrequest_approve
        )
    ...
    def _login(self):
        ...
        if not self.session:
            ...
            if self.client_cert and self.cert_passphrase:
               self.logger.debug("CAhandler._login() using PKCS12 client authentication")
               self.session.mount(
                    self.api_host,
                    Pkcs12Adapter(
    ...
    def _revoke(self, serial: str) -> Tuple[int, str, str]:
        ...
        if not str(serial).lower().startswith("0x"):
            serial = f"0x{serial}"

        # we need a nonce first to lets send a get to the certificate status endpoint to get the nonce
        code, response = self._api_get(f"{self.REST_AGENT_CERTS}{serial}")

        if code == 200 and isinstance(response, dict):
            if "Nonce" in response:
                nonce = response["Nonce"]
                self.logger.debug("CAhandler._revoke(): received nonce for revocation: %s", nonce)
                if 'Status' in response and response['Status'].lower() == 'valid':
                    self.logger.debug("CAhandler._revoke(): Certificate with serial %s is valid, proceeding with revocation", serial)
                ...
                elif 'Status' in response and response['Status'].lower() == 'revoked':
            ...
            return (code, "Failed to get certificate status before revocation attempt", str(response))
    ...
    def enroll(self, csr: str) -> Tuple[str, str, str, str]:
        ...
        error: Optional[str] = eab_profile_header_info_check(self.logger, self, csr, "profile_id")
    ...
    def handler_check(self):
        ...
        error = handler_config_check(
            self.logger, self, ["api_host", "client_cert"]
        )
